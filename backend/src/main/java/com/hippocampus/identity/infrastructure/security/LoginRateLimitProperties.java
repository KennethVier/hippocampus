...
public record LoginRateLimitProperties(int maxAttempts, Duration window, int maximumTrackedAddresses) {
    public LoginRateLimitProperties {
        if (maxAttempts < 1 || window == null || window.isNegative() || window.isZero()
                || maximumTrackedAddresses < 1) throw new IllegalArgumentException("Invalid login rate-limit configuration");
    }
}
