...
public record GoldenRetrievalDataset(
    ...
    public void validate() {
        ...
        for (Case c : cases) {
            ...
            if (c.query() == null || c.query().isBlank()) throw new IllegalArgumentException("query must not be blank for case " + c.id());
            ...
            if (c.acceptableAlternativeChunkKeys() == null) throw new IllegalArgumentException("acceptable chunks must be provided for case " + c.id());
            if (c.irrelevantChunkKeys() == null) throw new IllegalArgumentException("irrelevant chunks must be provided for case " + c.id());
