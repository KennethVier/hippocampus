...
- **Authority:** Documents 21,25
- **Evidence / link:** Implementation PR [#103](https://github.com/KennethVier/hippocampus/pull/103) on branch `codex/plan-pdf-metadata/native-text-extraction` adds a framework-free `ExtractPdfNativeText` capability, authoritative MaterialVersion source resolution, provider-neutral `BinaryObjectStore` temporary-file staging, staged-file Tika MIME reinspection, PDFBox 3.0.8 file-backed loading, immutable 1-based page/batch contracts, configurable page/batch/native-text limits, and bounded-writer enforcement while PDFBox emits each page. Deterministic tests cover a five-page native PDF including a blank page and final partial batch, a runtime-generated 601-page PDF consumed by scalar-only counters in 19 batches at batch size 32 with a final 25-page batch, per-page native-text resource exhaustion, page-limit rejection, password protection, malformed PDF, staged MIME mismatch, missing object, partial download, sink abort, and cleanup success/failure precedence. Focused P3-04, existing non-PostgreSQL materials/P3-02/P3-03, upload-regression, configuration, and architecture tests pass locally. The resolved dependency tree contains existing `org.apache.tika:tika-core:4.0.0` plus direct `org.apache.pdfbox:pdfbox:3.0.8` and its `pdfbox-io`/`fontbox` 3.0.8 transitives only. The initial implementation head `e7065dc3d2dce6b74b046891fd62cfbc0500a6df` passed authoritative PR quality run [#255](https://github.com/KennethVier/hippocampus/actions/runs/33736021060) (ID `33736021060`) with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` all successful; `backend-quality` executed the PostgreSQL/Testcontainers source-resolution integration tests, migrations, architecture tests, and backend build, resolving the implementation-time Docker limitation, and the PR `security` job passed explicit commit-range secret scanning plus introduced-dependency review for PDFBox. Initial external general implementation review verdict was **APPROVED WITH REQUIRED CHANGES** (review ID `5099918168`): it required moving `PdfExtractionException` from `materials.application` to the inward `materials.port` contract and correcting this evidence. The remediation moves the exception contract inward, updates production/test imports, and adds a permanent ArchUnit rule preventing `persistence`, `pdf`, `inspection`, and `storage` infrastructure adapters from depending upward on application while allowing infrastructure configuration to compose application use cases. No migration, page-count persistence, raw extraction artifact, DocumentNode/TextBlock schema, OCR/classification/normalization/visual/chunk/progress work, or production `MATERIAL_EXTRACT` handler was introduced. Durable TextBlock persistence and production `MATERIAL_EXTRACT` registration remain P3-05-owned; P3-04 does not claim end-to-end durable extraction-stage success. Final general re-review, independent security review, exact-remediation-head CI, merge, and post-merge validation remain pending and are not claimed.
- **Notes / blockers:** No current environment blocker remains; final general re-review, independent security review, and exact-remediation-head CI remain pending completion gates rather than blockers. P3-05 remains responsible for durable TextBlock output and production `MATERIAL_EXTRACT` handler registration.
