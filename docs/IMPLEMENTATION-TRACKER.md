...
- **Evidence / link:** P4-11 implemented the protected local developer retrieval inspector at `POST /api/dev/rag/inspect`. The controller and composition are gated by both the `local` Spring profile and `hippocampus.rag.inspector.enabled=true`; only `application-local.yml` enables it, while default and pilot profiles keep the component absent. The narrow request accepts only `topicId`, exact unmodified `query`, and `groundingMode`, with required/nonblank/bounded validation and no caller-owned user, Material, MaterialVersion, DocumentNode, generation, vector, weight, limit, or arbitrary-scope input. `InspectRetrieval` always derives authorization first through `BuildRetrievalScope.Query(topicId, groundingMode)` and therefore `CurrentUser`; an empty scope returns an immutable empty diagnostic without lexical search, active-generation lookup, provider invocation, or vector search. Lexical, vector, and hybrid projections preserve repository/merger ordering and expose only candidate identifiers, provenance, headings/pages, source quality, ranks, scores/signals, typed vector availability, optional active-generation metadata, counts, and non-negative total duration; repository hit/candidate source content remains internal to the existing retrieval/merge objects and is never copied into RetrievalInspection or the HTTP response. `ActiveIndexGenerationRepository` and `JdbcActiveIndexGenerationRepository` read only `ACTIVE` generations, return empty for zero, return the single generation for one, and fail closed for multiple active generations without reusing the embedding-job BUILDING fallback. Optional query embedding submits exactly one unchanged query under a fresh correlation UUID and validates result count, reference correlation, exact provider/model/version/dimension metadata, dimension, finiteness through the existing embedding value contract, and non-zero norm before vector search; no provider, no active generation, incompatibility, invalid output, and known provider failure remain typed lexical-only states without exposing raw provider errors. Existing `HybridCandidateMerger` policy is reused unchanged and supports lexical-only/vector-only/hybrid results. The existing `/api/**` authenticated-session boundary and normal CSRF protection remain unchanged: integration tests prove unauthenticated `401 AUTHENTICATION_REQUIRED`, authenticated missing-CSRF `403 CSRF_VALIDATION_FAILED`, and authenticated+CSRF success. Focused P4-11 application, PostgreSQL, configuration/profile, endpoint security, production-composed smoke, centralized retrieval-scope regression, and architecture validation passed 28 tests; the causal profile-start regression additionally passed with the endpoint tests after prerequisite-safe lazy wiring. The production-composed PostgreSQL smoke used synthetic `posterior cord` fixtures and a fake `EmbeddingPort`, visibly returned the authorized chunk ID, `Posterior Cord` heading, page 12, STRONG quality, lexical/vector/hybrid ranking signals and active generation, excluded an otherwise-better foreign user's vector candidate, and omitted both private source-text sentinels. All 11 architecture tests passed. Required `node scripts/validation/validate.mjs backend` returned **VALIDATION PASS** with 937 tests, 0 failures, 0 errors, and 9 skips; `git diff --check` passed. General implementation/diff review found no unresolved blocker. The local/pre-PR scoped security review reported **SECURITY PASS**, while the mandatory independent exact-head security review remains pending, covering access control, CSRF/session behavior, server-derived scope, authorization-before-ranking, empty-scope fail-closed behavior, cross-user isolation, parameterized SQL, provider/error sanitization, response/log data minimization, local/default/pilot configuration, secrets, and supply-chain impact; no dependency or migration was added. No P4-12 Golden Retrieval Dataset/quality thresholds, P4-13 release suite, production retrieval orchestration, query expansion/reranking policy, prompt/AI generation, frontend, admin role, source preview/delivery, signed URL, external service, or later-phase behavior was introduced. Implementation is committed and published in PR #165; P4-11 remains Ready for Review pending the mandatory independent exact-head security review, exact-head CI, merge, and separate tracker completion.
- **Notes / blockers:** _None_
