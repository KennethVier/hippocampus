...
- **Authority:** Documents 21
- **Evidence / link:** P4-04 implements the durable provider-neutral `EMBED` stage. `EmbedMaterialVersion` reads only active chunks for one material version in deterministic `chunk_index` order, submits bounded batches of unchanged canonical content through `EmbeddingPort` with chunk UUID correlation, verifies provider cardinality/reference/model/dimension compatibility, checks processing-job ownership before and after each provider call, and reports durable embedded/eligible counts. Provider calls run outside database transactions; each successful batch is persisted by the short transactional `PersistEmbeddingBatch` collaborator through `JdbcEmbeddingJobRepository`. PostgreSQL persistence uses generation-scoped `ON CONFLICT (chunk_id,index_generation_id) DO NOTHING`, validates every chunk is active and belongs to the requested material version, preserves an existing valid row on replay, and retains the V15 pgvector dimension trigger. Generation selection prefers exactly one `ACTIVE`, otherwise exactly one `BUILDING`, fails closed on ambiguity or incompatibility, and concurrency-safe initial creation records actual first-response provider/model/version/dimension metadata with `CHUNKER_V1`. Processing progression is now handler-availability based: `CHUNK` can durably enqueue `EMBED` only when its handler is present, while `EMBED` stops before unavailable `INDEX`; normal startup remains valid without an enabled `EmbeddingPort`. Stable recovery classification maps provider failure to transient `EMBEDDING_PROVIDER_UNAVAILABLE` and invalid response to fatal `EMBEDDING_RESPONSE_INVALID`. Deterministic tests prove 7-at-3 batching (`3/3/1`), canonical input/correlation, durable progress, inactive exclusion, provider calls outside transactions, partial-success durability and retry of only missing chunks, zero-call replay idempotency, malformed/mismatched response rejection, ownership-loss fencing, compatible concurrent initial generation creation, pgvector persistence/dimension enforcement, generation-scoped uniqueness, fail-closed ambiguous generation selection, mixed-material batch rejection with complete transaction rollback, and real `CHUNK -> EMBED` completion without speculative `INDEX`. The focused PostgreSQL/Testcontainers persistence suite passed 9 tests. Required `node scripts/validation/validate.mjs backend` returned **PASS**: clean Maven verification executed 806 tests with 0 failures, 0 errors, and 9 skips; `git diff --check` passed. General implementation review returned **APPROVED** after ownership and chunking-version hardening. The independent adversarial security gate returned **SECURITY PASS** with no unresolved blocking finding in the P4-04 scope; material/version scoping is enforced at persistence, provider output is treated as untrusted and contract-validated, SQL values are parameterized, successful prior batches remain durable without holding transactions across external calls, and the new mixed-material regression test demonstrates fail-closed atomic rollback. No schema migration, new dependency, credential exposure, provider-specific exception handling, retrieval/index/activation/reindex implementation, frontend work, or P4-05+ behavior was introduced. Final validation verdict: **VALIDATION PASS**.
- **Notes / blockers:** _None_
