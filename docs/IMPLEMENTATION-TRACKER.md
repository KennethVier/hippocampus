...
- **Definition of Done:** Tests pass.
- **Authority:** Documents 21
- **Evidence / link:** P3-12 is implemented in the current working tree. The unchanged durable processing sequence remains `MATERIAL_VALIDATE -> MATERIAL_EXTRACT -> STRUCTURE_DETECT -> VISUAL_EXTRACT -> NORMALIZE -> CHUNK -> EMBED`; inside `VISUAL_EXTRACT`, table extraction now runs after P3-10 visual extraction and P3-11 visual-context association. A provider-neutral `PdfTableExtractor` boundary and PDFBox adapter inspect bounded transient page geometry one physical page at a time, sort positions once, group baselines/cells linearly, discard page geometry before advancing, and never persist glyph coordinates. Stable native layouts require at least three rows and three consistently anchored columns before producing `TABLE_TEXT / NATIVE / STRONG` with TAB-separated columns and LF-separated rows. Irregular, merged-like, or spanning-heading layouts preserve bounded source-order text as `LIMITED` without invented TAB relationships; two-column textbook prose and pages without credible table evidence produce no table block. OCR never uses whitespace geometry: only repeated, stable TAB or pipe delimiters already present in PAGE_TEXT may produce table evidence; STRONG/LIMITED OCR becomes TABLE_TEXT/LIMITED, POOR OCR remains POOR, and quality is never upgraded. Medical notation is preserved verbatim. Multiple same-page and identical cross-page occurrences remain distinct and cross-page segments are never stitched. TABLE_TEXT ordinals are deterministic and disjoint from PAGE_TEXT: `pageCount + globalTableSourceOrder`, ordered by physical page then table position. Every table uses the reusable fail-closed `DocumentNodePageLocator`, preserving the deepest containing same-version node with the document root as fallback and rejecting duplicate IDs, missing/duplicate roots, cycles, missing parents, and cross-version nodes. Typed positive limits bound tables per page/document, rows, columns, text characters, PDF pages, native characters, and text positions. PDF download, inspection, parsing, geometry analysis, and temporary-file cleanup run outside database transactions; each table write and final verification uses a short transactional boundary. JDBC persistence is parameterized, idempotent for exact replay, resumable after partial output, and fail-closed for ordinal/content/method/quality/node/page/version conflicts or unexpected stale TABLE_TEXT. Finalization verifies the exact owned count, contiguous ordinal range, page range, extraction-method/quality combinations, and same-version node references, including the zero-table case. P3-04 finalization now scopes its exact page-set summary to `block_type = 'PAGE_TEXT'`, so MATERIAL_EXTRACT replay neither counts nor mutates downstream TABLE_TEXT. No migration, dependency, new processing stage/job type, table aggregate/schema, synthetic visual, OCR redesign, normalization, chunking, embedding, RAG, frontend behavior, or architectural deviation was introduced. Final focused validation passed 44 tests with zero failures, errors, or skips, including `HippocampusArchitectureTests`. Formal general review, independent adversarial security review, authoritative Linux CI, merge, and completion evidence remain required before Done.
- **Notes / blockers:** _None_
