...
- **Evidence / link:** Boot-managed `spring-boot-starter-session-jdbc` (Spring Session 4.1.1) is configured with Flyway V4's standard PostgreSQL Spring Session schema, a configurable 30-minute idle timeout, and profile-scoped cookie policy (local/test HttpOnly + SameSite=Lax + Path=/api; pilot Secure + SameSite=None). Testcontainers-backed real-server integration tests prove authenticated session persistence, session-ID rotation, database-backed restart survival, and deterministic expiry rejection; migration tests assert tables, indexes, and cascade foreign key; profile tests assert cookie settings. Focused validation (`AuthenticationFlowIntegrationTests,SpringSessionJdbcIntegrationTests,CsrfProtectionIntegrationTests`) passed with 17 tests and zero failures/errors/skips; full backend `clean verify` passed with 63 tests and zero failures/errors/skips. Authentication-flow documentation updated in `docs/security/authentication-flow.md`.
- **Notes / blockers:** _None_
...
**Phase Gate Evidence:** P2-01 through P2-13 are Done. The explicit Phase 2 gate was implemented in PR [#94](https://github.com/KennethVier/hippocampus/pull/94): the PostgreSQL-backed integration gate composes Subject/Topic creation, upload/storage/persistence, the owner-scoped MaterialTopicLink application boundary, cross-user denial, logical deletion, and deleted-Material fail-closed behavior; the real Playwright journey proves the corresponding student-facing private organization/upload/delete lifecycle. Exact-final-head PR quality run [#227](https://github.com/KennethVier/hippocampus/actions/runs/33596931476) (ID `33596931476`) succeeded with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` all successful, and its PR `security` job passed explicit commit-range secret scanning and introduced-dependency review. External general implementation review verdict is **APPROVED** (PR comment ID `5505109346`) and independent adversarial security review verdict is **SECURITY PASS** (PR comment ID `5505113377`), with no unresolved blocking Critical, High, or Medium finding. PR #94 merged into `main` as `a58e48251fefb6a74d635089296a883c9c10418c`. Post-merge `main` quality run [#228](https://github.com/KennethVier/hippocampus/actions/runs/33603700687) (ID `33603700687`) succeeded on that exact merge commit with all six jobs successful. The push-run introduced-dependency-review step was skipped and is not counted as passed; the applicable introduced-dependency review passed on PR run #227. Document 26 Phase 2 exit criteria are satisfied: a student can privately organize subjects, topics, and uploaded medical learning materials. Applicable Phase 2 security/observability requirements are working, no undocumented architecture deviation is known, and no current Phase 2 blocker remains.
...
- **Authority:** Documents 26
- **Evidence / link:** Implementation PR [#94](https://github.com/KennethVier/hippocampus/pull/94) adds gate-only acceptance evidence with no production behavior change. Exact reviewed implementation head `f33bd6cf1d845e64d6d6e89fde3ef3f4043e583b` adds a PostgreSQL-backed `Phase2EndToEndGateIntegrationTests` that composes the real Subject/Topic APIs, upload/storage/persistence path, the existing owner-scoped `CreateUserSelectedMaterialTopicLink` application use case, User A/User B cross-user denial, logical Material deletion, and deleted-Material fail-closed linking; MaterialTopicLink is intentionally exercised at the application boundary because Phase 2 has no student-facing link endpoint/UI and no test-only transport was introduced. The same head adds one coherent real-browser Playwright journey in `phase2.spec.ts`: User A creates a Subject and Topic and uploads a Material; User B receives sanitized not-found responses for the foreign Subject/Material and cannot see User A names in owner-scoped lists; User A then deletes the Material and it becomes immediately unavailable, with browser authentication state absent from local/session storage. A new fail-closed `phase2-gate` GitHub Actions job requires `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, and the existing `phase1-gate` to succeed. Authoritative PR quality run [#226](https://github.com/KennethVier/hippocampus/actions/runs/33596085630) (ID `33596085630`) succeeded at the exact implementation head with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` all successful; backend logs confirm `Phase2EndToEndGateIntegrationTests` executed 1 test with 0 failures/errors/skips inside a successful 260-test backend build, and auth-e2e logs confirm the new `phase2.spec.ts` journey executed non-skipped as part of 9 passing Playwright tests. The PR security job passed explicit commit-range secret scanning and introduced-dependency review. External general implementation review verdict: **APPROVED**, PR comment ID `5505109346`. Independent adversarial security review verdict: **SECURITY PASS**, PR comment ID `5505113377`, with no unresolved blocking Critical, High, or Medium finding in the changed gate surface. No dependency, migration, production Java/API, frontend feature behavior, ProcessingJob, Phase 3 ingestion, RAG/AI, or undocumented architecture deviation was introduced; no ADR is required. Final PR head `c35b6ab2e605def3b6d7f272422ba517aed68b5b` passed exact-final-head quality run [#227](https://github.com/KennethVier/hippocampus/actions/runs/33596931476) (ID `33596931476`) with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` all successful; the PR `security` job passed explicit commit-range secret scanning and introduced-dependency review. PR #94 merged into `main` as `a58e48251fefb6a74d635089296a883c9c10418c`. Post-merge `main` quality run [#228](https://github.com/KennethVier/hippocampus/actions/runs/33603700687) (ID `33603700687`) succeeded on that exact merge commit with all six jobs successful. The push-run introduced-dependency-review step was skipped and is not counted as passed; the applicable introduced-dependency review passed on PR run #227. The external general implementation review remains **APPROVED** (PR comment ID `5505109346`) and the independent adversarial security review remains **SECURITY PASS** (PR comment ID `5505113377`). The P2-13 expected private organization/upload lifecycle behavior and phase-gate Definition of Done are satisfied, Document 26 Phase 2 exit criteria are met, no current blocker remains, and no ADR or undocumented architecture deviation is required.
- **Notes / blockers:** _None_
...
- **Evidence / link:** P3-01 durable ProcessingJob persistence foundation was implemented in PR [#96](https://github.com/KennethVier/hippocampus/pull/96). The final reviewed PR head `9d6950c31c9c23db27c9bc932bb1df632469c7f8` includes additive Flyway migration `V8__create_processing_jobs.sql`, finite ProcessingJob status/type vocabularies, the `ProcessingJobEntity`, PostgreSQL/JPA schema and persistence coverage, heartbeat storage, `processing_version`, restrictive `users` / `material_versions` foreign keys, numeric/attempt constraints, active-only `(material_version_id, job_type, processing_version)` uniqueness, and durable identity immutability while deliberately deferring job claiming, worker execution, dispatching, extraction/OCR/chunking/embedding/indexing, APIs, frontend work, and P3-02+ behavior. Exact-head PR quality run [#235](https://github.com/KennethVier/hippocampus/actions/runs/33648345540) (ID `33648345540`) succeeded on that exact head with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` successful; its PR `security` job passed explicit commit-range secret scanning and introduced-dependency review. Final general implementation review verdict is **APPROVED** (review ID `5091819222`) and the independent adversarial security review verdict is **SECURITY PASS** (review ID `5091827297`), with no unresolved blocking Critical, High, or Medium finding. PR #96 merged into `main` as `6caf06fdb764320642930bf4fed6539466080079`. Post-merge `main` quality run [#236](https://github.com/KennethVier/hippocampus/actions/runs/33650463984) (ID `33650463984`) completed successfully on that exact merge commit with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` all successful. The push-run introduced-dependency-review step was skipped and is not counted as passed; the applicable introduced-dependency review passed on PR run #235. P3-01 Definition of Done is satisfied: the schema/migration/constraint and persistence tests are green, no new dependency or undocumented architecture deviation was introduced, and no ADR is required. Phase 3 remains unevaluated because its phase gate is owned by P3-18.
- **Notes / blockers:** _None_
...
- **Evidence / link:** P3-02 atomic PostgreSQL job claiming was implemented in PR [#99](https://github.com/KennethVier/hippocampus/pull/99). The final reviewed PR head `ded5b7763a6df828ce26521ce177113eadc40124` implements a short proxy-safe application-owned claim transaction, immutable `ClaimedProcessingJob` domain result, narrow claim port, and one parameterized PostgreSQL CTE using `FOR UPDATE ... SKIP LOCKED` to atomically claim eligible `PENDING` jobs, increment `attempt_count`, set `RUNNING` / lock metadata / first `started_at`, preserve heartbeat, exclude jobs for already-`DELETED` Materials, and select deterministically by `created_at ASC, id ASC`; additive Flyway V9 adds the PENDING/attempt-eligible FIFO claim index. Real PostgreSQL/Testcontainers coverage separately proves exactly-one ownership under a two-worker same-job race and genuine `SKIP LOCKED` forward progress by holding the first candidate locked while another worker claims the next row, plus ineligible-state, attempt-limit, deleted-Material, null-version, FIFO, worker-ID validation, migration/index, transaction-proxy, and failure-propagation behavior. Exact-final-head PR quality run [#242](https://github.com/KennethVier/hippocampus/actions/runs/33666033867) (ID `33666033867`) succeeded on `ded5b7763a6df828ce26521ce177113eadc40124` with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` successful; its PR security job passed explicit commit-range secret scanning and introduced-dependency review. Final general implementation review verdict is **APPROVED** (review ID `5093494653`) and the independent adversarial security review verdict is **SECURITY PASS** (review ID `5093500730`), with no unresolved blocking Critical, High, or Medium finding in P3-02 scope. PR #99 merged into `main` as `1fb53f9b90d4231b88ecd21ca8e17c8b08f18362`. Post-merge `main` quality run [#243](https://github.com/KennethVier/hippocampus/actions/runs/33668615619) (ID `33668615619`) completed successfully on that exact merge commit with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` successful. The post-merge push-run introduced-dependency-review step was skipped and is not counted as passed; the applicable introduced-dependency review passed on PR run #242. P3-02 Definition of Done is satisfied: PostgreSQL atomic claiming and two-worker concurrency / explicit `SKIP LOCKED` behavior are green, no new dependency or undocumented architecture deviation was introduced, no blocker remains, and ADR is not required. P3-03 remains `Not Started`; Phase 3 remains unevaluated because the phase gate is owned by P3-18.
- **Notes / blockers:** _None_
...
- **Evidence / link:** P3-03 processing dispatch and successful stage chaining were implemented in PR [#101](https://github.com/KennethVier/hippocampus/pull/101). The final reviewed implementation head `59fb074a8fdcf7ee9c5011b60ec3e1c8c566fa7f` provides a framework-free six-stage dispatcher/handler contract, central `MATERIAL_VALIDATE → MATERIAL_EXTRACT → STRUCTURE_DETECT → VISUAL_EXTRACT → NORMALIZE → CHUNK → EMBED` sequence, fail-closed missing/unsupported/duplicate routing, separate proxy-safe successful-stage completion, and a narrow PostgreSQL lifecycle adapter that atomically changes the matching `RUNNING` job to `COMPLETED` and creates exactly one next Phase 3 `PENDING` job from authoritative row data. `CHUNK` completes while exposing `EMBED` only as next-stage metadata and does not create Phase 4 work. MaterialVersion-backed claim eligibility is owner-qualified before handler execution with `processing_jobs.user_id = materials.user_id`; PostgreSQL regression coverage proves malformed cross-owner work is skipped while valid same-owner work remains claimable. Dispatcher/handler contract tests cover all six routes plus fail-closed, null-version, exception, processing-version, and non-recursive behavior; real PostgreSQL/Testcontainers lifecycle tests cover successful chaining/copying, atomic rollback, bounded concurrent duplicate completion, wrong status, wrong type, and the CHUNK boundary. Exact-final-head PR quality run [#251](https://github.com/KennethVier/hippocampus/actions/runs/33677512735) (ID `33677512735`) succeeded on `59fb074a8fdcf7ee9c5011b60ec3e1c8c566fa7f` with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` successful; its PR security job passed explicit commit-range secret scanning and introduced-dependency review. Final general implementation review verdict is **APPROVED** (review ID `5094816900`) and the independent adversarial security review verdict is **SECURITY PASS** (review ID `5094822456`), with no unresolved blocking Critical, High, or Medium finding in P3-03 scope. PR #101 merged into `main` as `a9de13751077bee5aae797ea7dec2c43d3c6024c`. Post-merge `main` quality run [#252](https://github.com/KennethVier/hippocampus/actions/runs/33680983555) (ID `33680983555`) completed successfully on that exact merge commit with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` successful. The post-merge push-run introduced-dependency-review step was skipped and is not counted as passed; the applicable introduced-dependency review passed on PR run #251. P3-03 Definition of Done is satisfied: handler contract/routing and successful durable stage chaining are green, owner isolation is enforced before processing, no production no-op stage handlers or P3-04/P3-15 scope leakage were introduced, no migration/dependency or undocumented architecture deviation exists, no blocker remains, and ADR is not required. P3-04 remains `Not Started`; Phase 3 remains unevaluated because P3-18 owns the phase gate.
- **Notes / blockers:** _None_
...
- **Authority:** Documents 21,25
- **Evidence / link:** P3-04 bounded PDF metadata/native-text extraction was implemented in PR [#103](https://github.com/KennethVier/hippocampus/pull/103). The final reviewed implementation head `6f08ce4b0ed3c22285734227f152a935c40186e0` provides the framework-free `ExtractPdfNativeText` capability, authoritative current MaterialVersion source resolution, provider-neutral `BinaryObjectStore` staging to backend-generated temporary files, staged-byte Tika MIME reinspection, PDFBox 3.0.8 file-backed loading, immutable 1-based page/batch contracts, configurable page/batch/native-text resource limits, bounded-writer enforcement during `PDFTextStripper` output, typed fail-closed handling for missing/deleted/non-PDF sources, partial downloads, MIME mismatch, encrypted/corrupt PDFs, page/resource limits, sink failure, and cleanup failure, plus a permanent ArchUnit rule preventing PDF/persistence/inspection/storage adapters from depending upward on application. Deterministic tests cover a five-page native PDF including a blank page/final partial batch and a runtime-generated 601-page PDF consumed incrementally in 19 batches at batch size 32 with a final 25-page batch, without a whole-file byte array, whole-document String, or retained all-page collection. Exact-final-head PR quality run [#267](https://github.com/KennethVier/hippocampus/actions/runs/33741615289) (ID `33741615289`) completed successfully on `6f08ce4b0ed3c22285734227f152a935c40186e0` with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` all successful; its PR security job passed explicit commit-range secret scanning and introduced-dependency review for PDFBox. Final external general implementation review verdict is **APPROVED** (review ID `5102649199`) and the independent adversarial security review verdict is **SECURITY PASS** (review ID `5102660863`), with no unresolved blocking finding in P3-04 scope. PR #103 merged into `main` as `86064e39accd51fd8a24c7c55a8e12b832716621`. Post-merge `main` quality run [#268](https://github.com/KennethVier/hippocampus/actions/runs/33765026056) (ID `33765026056`) completed successfully on that exact merge commit with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` all successful. The post-merge push-run introduced-dependency-review step was skipped and is not counted as passed; the applicable introduced-dependency review passed on PR run #267. P3-04 Definition of Done is satisfied: native fixture output, ordered bounded extraction, the 601-page large-PDF case, resource/failure behavior, provider-neutral staging, architecture direction, and exact-head/post-merge validation are green. External review explicitly approved the P3-04/P3-05 ownership boundary: P3-04 completes the bounded extraction/sink capability, while durable `DocumentNode`/`TextBlock` persistence, page-count persistence in the real extraction flow, and production `MATERIAL_EXTRACT` handler registration remain P3-05-owned. No migration, raw extraction-artifact architecture, OCR/classification/normalization/visual/chunk/progress scope leakage, undocumented architecture deviation, or blocker remains, and ADR is not required. P3-05 remains `Not Started`; Phase 3 remains unevaluated because P3-18 owns the phase gate.
- **Notes / blockers:** _None_
...
- **Authority:** Documents 18,21
- **Evidence / link:** P3-05 durable DocumentNode/TextBlock persistence was implemented in PR [#105](https://github.com/KennethVier/hippocampus/pull/105). Final reviewed implementation head `82dc8ec5dbb0e8e229150b424526ab1ed67def64` adds additive Flyway `V10__create_document_structure.sql`, JPA mappings and bounded query repositories for `DocumentNode`/`TextBlock`, lifecycle-qualified JDBC batch/finalization persistence, and the production `MATERIAL_EXTRACT` handler composed through the existing P3-03 dispatcher. Native P3-04 pages persist exactly once as globally ordered `PAGE_TEXT` blocks (`ordinal = page_number`, `extraction_method = NATIVE`, nullable quality), including blank pages; composite foreign keys reject cross-MaterialVersion hierarchy/provenance; exact replay preserves row identity/timestamps; conflicting replay and incomplete/conflicting finalization fail closed; and finalization records only the verified P3-04 `page_count` and DOCUMENT root range. Tests cover migration/schema metadata, hierarchy acyclicity, PostgreSQL constraints, multi-batch atomicity, replay/rollback, lifecycle deletion/concurrency, transaction topology, bounded repository queries, and real PDFBox extraction followed by unchanged P3-03 completion. Exact-final-head PR quality run [#280](https://github.com/KennethVier/hippocampus/actions/runs/34005574101) (ID `34005574101`) succeeded with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` successful; `backend-quality` ran backend tests, architecture tests, migrations, and build, and the PR `security` job passed explicit commit-range secret scanning and introduced-dependency review. External general implementation review verdict is **APPROVED** and independent adversarial security review verdict is **SECURITY PASS**, recorded in review ID `5123815593`, with no unresolved blocking finding in P3-05 scope. PR #105 merged into `main` as `93c3377edd3844ab2a371842a5f6f6afbe1b0d67`. Post-merge `main` quality run [#281](https://github.com/KennethVier/hippocampus/actions/runs/34007721921) (ID `34007721921`) succeeded on that exact merge commit with all required jobs successful; `backend-quality` again passed backend tests, architecture tests, migrations, and build. The push-run introduced-dependency-review step was skipped and is not counted as passed; the applicable introduced-dependency review passed on PR run #280. The expected queryable durable source-structure behavior and migration/repository-test Definition of Done are satisfied, no current blocker remains, semantic normalization remains P3-13-owned, Phase 3 remains unevaluated because P3-18 owns the phase gate, and no ADR or undocumented architecture deviation is required.
- **Notes / blockers:** _None_
...
- **Authority:** Documents 21
- **Evidence / link:** Implementation PR [#112](https://github.com/KennethVier/hippocampus/pull/112) delivered deterministic `NATIVE_TEXT` / `MIXED` / `IMAGE_ONLY` / `UNREADABLE` page classification in the bounded PDF extraction stream. The final reviewed implementation head `0197796d464700d51bf85bdcb27dbf1960a0d016` uses provider-neutral classification with Unicode-aware meaningful-text handling and PDFBox-local painted-image inspection without page rendering, image extraction, image persistence, OCR implementation, or schema changes. Deterministic coverage includes native/blank pages, mixed and scanned/image-only pages, tiny incidental text, Unicode separator/noise boundaries, direct/multiple/nested-form/inline painted images, unused resources, and the existing 601-page bounded extraction regression. Exact-head PR quality run [#285](https://github.com/KennethVier/hippocampus/actions/runs/34011544388) (ID `34011544388`) succeeded, including `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate`; the PR `security` job passed explicit commit-range secret scanning and introduced-dependency review. External general implementation review verdict is **APPROVED** and independent adversarial security review verdict is **SECURITY PASS**, recorded in review ID `5124229842`, with no unresolved blocking Critical, High, or Medium finding in P3-06 scope. PR #112 merged into `main` as `cd6f4d94312a118fa9c387353f84a6f8882f949d`. Post-merge `main` quality run [#286](https://github.com/KennethVier/hippocampus/actions/runs/34013104976) (ID `34013104976`) succeeded on that exact merge commit with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, and `phase2-gate` successful. The push-run introduced-dependency-review step was skipped and is not counted as passed; the applicable introduced-dependency review passed on PR run #285. P3-06 expected four-state page classification behavior and mixed/scanned classification Definition of Done are satisfied. No OCR/P3-07 implementation, migration, new dependency, undocumented architecture deviation, ADR requirement, or current blocker remains. P3-07 remains `Not Started`, and Phase 3 remains unevaluated because P3-18 owns the Phase 3 gate.
- **Notes / blockers:** _None_
...
- **Evidence / link:** Implementation PR [#114](https://github.com/KennethVier/hippocampus/pull/114) delivered the provider-neutral `OcrPort`, defensive bounded `OcrInput`, explicit recognized/no-usable-text `OcrResult`, OCR method/quality persistence and replay/finalization, conservative `IMAGE_ONLY`-only OCR routing, bounded grayscale PDFBox rasterization, and a local Tesseract CLI adapter using fixed `stdin stdout -l eng tsv` arguments. Final reviewed implementation head `529a24931cdb42021cf289e3354cf76e054ab39e` includes the final render-boundary resource-safety correction: authoritative source-image dimension, per-image-pixel, and page-pixel limits are enforced at PDFBox `PageDrawer.drawImage(PDImage)` before image decoding/delegation, with checked aggregate unique-image accounting and recursive hard/soft-mask validation covering normal page content, forms/nested forms, tiling patterns, and annotation appearance streams while retaining renderer subsampling, UserUnit-safe canvas preflight, encoded PNG limits, Tesseract I/O/text/TSV limits, timeout/forced termination, constrained subprocess environment, one-page-at-a-time processing, and page-resource cleanup. The documented Ubuntu runtime is Tesseract `5.3.4-1build5` (`tesseract 5.3.4`), `libtesseract5 5.3.4-1build5`, and `tesseract-ocr-eng 1:4.1.0-2`; `eng` availability and non-skipped real-engine strong/degraded/no-text behavior were validated. Exact-head PR quality run [#291](https://github.com/KennethVier/hippocampus/actions/runs/34038178527) (ID `34038178527`) succeeded with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, and `phase2-gate` successful; `backend-quality` ran backend tests, architecture tests, PostgreSQL/Testcontainers migrations/integration tests, real Tesseract validation, and the build. External general implementation review verdict is **APPROVED FOR COMMIT AND PR PUBLICATION**, recorded in review ID `5125590505`, and independent adversarial security review verdict is **SECURITY PASS**, recorded in review ID `5125593913`, with no unresolved blocking Critical, High, or Medium finding in P3-07 scope. PR #114 merged into `main` as `285ecbb91973f4ae5db98d3916e9383a7ff03aba`. Post-merge `main` quality run [#292](https://github.com/KennethVier/hippocampus/actions/runs/34039408980) (ID `34039408980`) succeeded on that exact merge commit with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, and `phase2-gate` all successful. The push-run introduced-dependency-review step was skipped and is not counted as passed; the applicable introduced-dependency review passed on PR run #291. P3-07 expected scanned-page OCR or explicit-limitation behavior and adapter/engine-documentation Definition of Done are satisfied. No migration, P3-08+, P3-10 visual extraction, P3-15 recovery, cloud OCR, deployment scope, ADR requirement, undocumented architecture deviation, or current blocker remains. Phase 3 remains unevaluated because P3-18 owns the Phase 3 gate.
- **Notes / blockers:** _None_
...
- **Authority:** Documents 21
- **Evidence / link:** Implementation PR [#116](https://github.com/KennethVier/hippocampus/pull/116) delivered deterministic P3-08 structure detection from durable `PAGE_TEXT` evidence plus a provider-neutral, PDFBox-backed supplementary inspection boundary. The implementation uses bounded bookmark/outline destinations, TOC corroboration, conservative font-weight/prominence and relative layout/spacing, numbering, and repeated positional-region signals; emits only `CHAPTER`/`SECTION`/`SUBSECTION` nodes with `NATIVE`/`HEURISTIC` provenance and `HIGH`/`MEDIUM`/`LOW` confidence; preserves the `DOCUMENT` root and `PAGE_TEXT` rows; and performs exact semantic replay or fail-closed conflict detection with atomic JDBC persistence in a separate short `@Transactional` application bean. PDF download, MIME reinspection, PDFBox parsing, page-local layout analysis, OCR uncertainty handling, and deterministic hierarchy calculation remain outside the write transaction. Resource ceilings cover page/batch size, outline count/depth/title/destination/cycles, text positions, aggregate retained native text per page, layout lines, candidates, and detected nodes; the synthetic 601-page fixture completed in batches no larger than 32. Final corrections deduplicate repeated header/footer signatures once per physical page, make native-outline/body confirmation confidence OCR-quality aware, lock the Material lifecycle row with the MaterialVersion and `DOCUMENT` root before detected-structure writes, and make structure auto-configuration back off when required `PdfExtractionProperties` are unavailable; a regression test covers that no-JDBC/property-missing context. The exact reviewed P3-08 implementation head `048f199e5aade052f9e66c6f465c835593466fc9` passed PR quality run [#306](https://github.com/KennethVier/hippocampus/actions/runs/34125361034) (ID `34125361034`) with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, and `phase2-gate` successful, including backend tests, architecture tests, PostgreSQL/Testcontainers migrations/integration tests, real Tesseract validation, and build. General implementation review is **APPROVED** and independent adversarial security review is **SECURITY PASS** for P3-08 scope, with no unresolved blocking Critical, High, or Medium finding; final completion review is recorded on PR #116 in comment ID `5571322476`. The final PR head `58155cf37c8c9702521385ce35844efab9365592` differs from the reviewed P3-08 head only by upstream agent/governance documentation files and contains no subsequent P3-08 implementation/configuration/test change. PR #116 merged into `main` as `5a6cb44dc2ae4609c3a87d224e8476ebd53c1617`. Post-merge `main` quality run [#308](https://github.com/KennethVier/hippocampus/actions/runs/34126698488) (ID `34126698488`) succeeded on that exact merge commit with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, and `phase2-gate` all successful. The push-run introduced-dependency-review step was skipped and is not counted as passed; the applicable introduced-dependency review passed on PR run #306. P3-08 expected known-heading/section detection with source order and fixture-hierarchy Definition of Done are satisfied. No P3-09+ implementation, new dependency, migration, ADR requirement, undocumented architecture deviation, or current blocker remains. Phase 3 remains unevaluated because P3-18 owns the Phase 3 gate.
- **Notes / blockers:** _None_
...
- **Evidence / link:** P3-09 bounded AI-assisted structure fallback was implemented in PR [#120](https://github.com/KennethVier/hippocampus/pull/120). The reviewed implementation head `d8f0b12ba3466d47712ec6ab318862369c620a0f` adds `StructureFallbackPolicy`, the provider-neutral `StructureFallback` port, bounded `StructureFallbackRequest`, versioned `StructureFallbackResponse`, `StructureFallbackValidator`, `ApplyStructureFallback`, and production `UnavailableStructureFallback`. The application invokes optional assistance after deterministic structure detection and before transactional persistence. Only LOW-confidence deterministic nodes are eligible, with at most eight attempts per document; HIGH/MEDIUM deterministic nodes and an empty detected hierarchy do not trigger assistance. Each request contains at most three contiguous local PAGE_TEXT pages around the ambiguous heading, with at most 2,000 characters per page. Contract `materials.structure-fallback.v1`, prompt `materials.structure-fallback.prompt.v1`, and response schema `materials.structure-fallback.response.v1` are explicit and versioned without introducing the Phase-5 Prompt Registry. Returned assistance is treated as untrusted: response versions, grounded heading text, page identity, node type, hierarchy compatibility, and duplicate conflicts are validated before conversion. Accepted recovery retains application-owned parent links, source order, page ranges, LOW confidence, and `AI_ASSISTED` provenance; unrelated deterministic nodes remain unchanged. Missing, unavailable, failed, timeout-like, malformed, version-mismatched, ungrounded, or otherwise invalid assistance preserves the deterministic P3-08 result and does not block durable structure extraction. Phase-3 production wiring uses `UnavailableStructureFallback`; no Gemini/Ollama adapter, AIProvider/ProviderRouter, Spring AI/provider SDK, Prompt Registry, real external call, new dependency, migration, frontend, RAG/embedding, or P3-10+ capability was introduced. Focused validation `mvn -B -ntp "-Dtest=*Structure*Tests,HippocampusArchitectureTests" test` passed 71 tests (62 structure and 9 architecture) with zero failures/errors/skips. Authoritative PR quality run [#311](https://github.com/KennethVier/hippocampus/actions/runs/34134322955) (ID `34134322955`) succeeded on the reviewed implementation head with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, and `phase2-gate` successful. The Linux `backend-quality` job installed the validated OCR runtime and successfully ran backend tests, architecture tests, PostgreSQL/Testcontainers migrations/integration tests, real Tesseract validation, and the backend build, supplying the authoritative full validation that the earlier Windows-local run could not provide. External general implementation review verdict is **APPROVED** with no unresolved implementation blocker. Independent adversarial security review verdict is **SECURITY PASS**, recorded in PR review ID `5133499316`, with no unresolved blocking Critical, High, or Medium finding in P3-09 scope. PR #120 merged into `main` as `a96dc14cd61a043eda92298d5c3378f013622111`. Post-merge `main` quality run [#313](https://github.com/KennethVier/hippocampus/actions/runs/34139756983) (ID `34139756983`) succeeded on that exact merge commit with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, and `phase2-gate` successful. The push-run introduced-dependency-review step was skipped and is not counted as passed; the applicable introduced-dependency review already passed on PR run #311. The Expected Result and Definition of Done are satisfied: the stub boundary is complete, deterministic extraction remains non-blocking when optional AI assistance is unavailable or invalid, and no premature provider coupling exists. Live provider enablement remains intentionally deferred to P5 and requires a fresh independent security review when activated. No ADR or undocumented architecture deviation is required, and no current blocker remains.
- **Notes / blockers:** _None_
...
- **Authority:** Documents 18,21
- **Evidence / link:** P3-10 was implemented in PR [#122](https://github.com/KennethVier/hippocampus/pull/122). The implementation adds the provider-neutral `PdfVisualExtractor` boundary and a real PDFBox adapter that walks actually painted page content one physical page at a time, including inline images and nested forms, while excluding unused resources and stencil masks as first-class assets. Exact standalone JPEG/JPX bytes are retained when safely available; other painted images are losslessly encoded as PNG. The application hashes the exact stored bytes with SHA-256, writes deterministic private-object keys under `materials/{materialVersionId}/visuals/{pageNumber}/{contentHash}.{suffix}`, deduplicates repeated same-page visual output, preserves occurrences across pages, and associates each asset with the deepest applicable same-version `DocumentNode` or the `DOCUMENT` root. Object-store and PDF-processing work remain outside the metadata transaction; `JdbcVisualAssetPersistence` performs a separate short fail-closed transaction with exact-replay convergence. Flyway `V11__create_visual_assets.sql` creates `visual_assets` with MaterialVersion ownership, same-version composite DocumentNode foreign-key enforcement, positive page/dimension constraints, controlled visual/interpretation vocabularies, unique storage keys, page/hash retry uniqueness, and supporting page lookup indexing. The real `VISUAL_EXTRACT` handler is registered without changing the existing `VISUAL_EXTRACT -> NORMALIZE` processing sequence. Initial focused validation passed 47 tests with zero failures/errors/skips, covering mixed visual fixtures, exact hashes and JPEG retention, inline/nested forms, unused resources, zero visuals, same-page and cross-page identity, extraction limits, hierarchy selection, unavailable/deleted source handling, transaction topology, replay/conflict behavior, migration constraints, stage-handler registration, sequencing, and architecture. A broader Windows-compatible validation run passed 436 tests with zero failures/errors and four intentional skips. A local Windows `mvn -B -ntp clean verify` attempt ran 464 tests but did not pass because five pre-existing PDF/OCR test classes depend on Unix paths, shell executables, Tesseract availability, or LF-only PDFBox text output; this remains accurately recorded as a Windows environment/platform limitation and is not reclassified as a pass. Initial external general implementation review at head `e45ff73ea420be69f23193ca77242de4781be3c7` identified two corrective issues: distinct painted source images with identical output could consume expensive extraction work before image/encoded-byte resource accounting, and exact P3-10 replay incorrectly treated downstream enrichment fields as extraction-owned metadata. Commit `c127e92034d62a92b045f0816e1a8f6e30a311f0` corrected both findings: each distinct painted source-image candidate is now charged against page/document image limits immediately after object-identity deduplication and produced encoded bytes are charged before content-hash deduplication; exact replay now verifies only extraction-owned page/hash identity, DocumentNode, storage key, and dimensions while preserving downstream caption, nearby-text, visual-type, and interpretation-status enrichment. Correction-focused validation passed 25 tests with zero failures/errors/skips, including identical-content distinct-XObject resource-limit coverage and enriched-row retry preservation. External general implementation re-review verdict is **APPROVED** with no unresolved implementation blocker. Authoritative PR quality run [#317](https://github.com/KennethVier/hippocampus/actions/runs/34150940193) (ID `34150940193`) succeeded on the exact final reviewed head `c127e92034d62a92b045f0816e1a8f6e30a311f0`, with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, and `phase2-gate` all successful; the PR `security` job also successfully completed explicit commit-range secret scanning and introduced-dependency review. Independent adversarial security review verdict is **SECURITY PASS**, recorded in PR review [#5136971125](https://github.com/KennethVier/hippocampus/pull/122#pullrequestreview-5136971125), with no unresolved blocking Critical, High, or Medium security finding in the reviewed P3-10 PDF visual-extraction, private-storage, and persistence surface. PR #122 merged into `main` as `aa93f58618181a3790e96f120ab964493f19abdd`. The Expected Result and Definition of Done are satisfied: source visuals are preserved as private first-class assets with physical-page and hierarchy provenance, exact stored-byte hashes, bounded processing, deterministic storage identity, and retry behavior that does not create duplicate durable assets or overwrite downstream enrichment. No new dependency, provider SDK, AI/vision integration, caption inference, visual interpretation, OCR/rendered-page fallback, sequencing change, frontend/RAG work, P3-11+ capability, ADR requirement, or undocumented architecture deviation was introduced. No current blocker remains.
- **Notes / blockers:** _None_
...
- **Authority:** Documents 21
- **Evidence / link:** P3-11 was implemented in PR [#124](https://github.com/KennethVier/hippocampus/pull/124). The implementation extends the existing `VISUAL_EXTRACT` handler so persisted P3-10 visuals are followed by deterministic visual-context association before the unchanged `NORMALIZE` transition. Association uses existing same-version `VisualAsset`, `DocumentNode`, and `PAGE_TEXT` evidence and intentionally remains conservative because persisted visual/text geometry is not available. Confident association requires exactly one visual and one trustworthy numbered `Figure`/`Fig.` caption candidate on the physical page. Number-only labels may use one bounded continuation line, while inline captions require an explicit period, colon, hyphen, en dash, or em dash; ordinary body-reference prose such as `Figure 4 shows ...` remains unresolved. Native page text and only strong-quality OCR are eligible for confident association; limited/poor OCR, multiple visuals, multiple caption candidates, absent captions, malformed/oversized evidence, and other ambiguous states remain unresolved rather than fabricated. Caption output is bounded to two lines / 300 characters and nearby explanatory context to three lines / 600 characters. The application validates visual and DocumentNode MaterialVersion provenance, groups visual metadata by page, loads and evaluates only one visual-bearing page's text at a time, and discards that page content before continuing so document-scale text is not accumulated. Durable enrichment is written through the short transactional `PersistVisualContext` boundary; the JDBC adapter re-reads and locks only the target visual row, verifies material-version/node/page provenance, fills only NULL `caption` and `nearby_text` fields, preserves existing/manual enrichment, creates no visual assets, and does not modify storage identity, content hash, page provenance, dimensions, visual type, or interpretation status. No migration, dependency, new processing stage, AI/provider integration, geometry extraction, table handling, RAG/chunk/embedding work, frontend behavior, or later tracker capability was introduced. Initial focused implementation validation passed 48 tests with zero failures/errors/skips. External general implementation review of initial head `c3b2823a079e29a0592578b821ce0b4f568aa702` identified three issues: OCR extraction quality was not considered; document-wide `PAGE_TEXT` accumulation and transaction/locking behavior were not sufficiently bounded; and the figure-caption grammar could treat body-reference prose as a caption. Commit `3682c46742a74c0113ab7af6be1ff5f0c11f39b0` corrected OCR evidence handling and introduced page-bounded processing with target-row transactional persistence. Commit `76fa7b7073f2008df7078281fb4cfedcc6bed0ac` tightened caption grammar so inline text requires an explicit delimiter and added body-reference regressions. Correction-focused validation passed 17 tests after the first correction and 23 tests after the final grammar correction, each with zero failures/errors/skips. External general implementation re-review verdict on final head `76fa7b7073f2008df7078281fb4cfedcc6bed0ac` is **APPROVED** with no unresolved implementation blocker. Authoritative PR quality run [#323](https://github.com/KennethVier/hippocampus/actions/runs/34189489276) (ID `34189489276`) succeeded on the exact final reviewed head, with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, and `phase2-gate` successful. Independent adversarial security review verdict is **SECURITY PASS**, recorded on PR #124 as review ID `5137809615`, with 0 Critical, 0 High, and 0 Medium findings and no unresolved blocking security issue in the reviewed P3-11 visual-context association and persistence surface. PR #124 merged into `main` as `a4d1272eb0cbe78affa9f4c943f1f50596e2f93e`. The Expected Result and Definition of Done are satisfied: trustworthy caption and bounded nearby-text context are attached when evidence is unambiguous, uncertain evidence remains unresolved, retry preserves existing enrichment, processing remains page-bounded and fail-closed, and the existing `VISUAL_EXTRACT -> NORMALIZE` processing sequence is unchanged. No ADR or undocumented architecture deviation is required. No current blocker remains.
- **Notes / blockers:** _None_
...
- **How it works:** Reliable native PDF table layouts preserve row/column semantics as `TABLE_TEXT`; uncertain or complex layouts preserve bounded source-order evidence with limited quality rather than fabricated cell relationships.
- **Dependencies:** TextBlocks + PDF extraction + DocumentNodes
- **Tests / validation:** Simple, multi-column, complex/failure, OCR, replay, provenance, resource-limit, and false-positive table fixtures.
- **Expected result:** Reliable tables searchable; uncertain tables marked limited.
- **Definition of Done:** Tests pass.
- **Authority:** Documents 21
- **Evidence / link:** P3-12 was implemented in PR [#126](https://github.com/KennethVier/hippocampus/pull/126). The existing durable processing sequence remains `MATERIAL_VALIDATE -> MATERIAL_EXTRACT -> STRUCTURE_DETECT -> VISUAL_EXTRACT -> NORMALIZE -> CHUNK -> EMBED`; inside `VISUAL_EXTRACT`, table extraction executes after P3-10 visual extraction and P3-11 visual-context association and before the unchanged `NORMALIZE` transition. P3-12 uses the existing `text_blocks` persistence model and `TextBlockType.TABLE_TEXT`; no migration, table/cell aggregate, new processing stage, dependency, AI/provider integration, normalization, chunking, embedding, RAG, or frontend behavior was introduced. Reliable native PDF table layouts may contain two or more columns and rows; conservative borderless `STRONG` confidence requires at least three consistent rows, stable anchors, compact table-like cells, and no overlapping, rotated, columnar-prose, or side-by-side ambiguity. Strong native tables serialize deterministically with TAB-separated columns and LF-separated rows while preserving source medical notation. Credible but structurally uncertain native layouts remain `LIMITED` and preserve source-order text without invented TAB relationships. Two-column and three-column textbook prose regressions prevent ordinary columnar content from being promoted to tables, and side-by-side/rotated/overlapping layouts cannot establish fabricated STRONG table relationships. OCR table handling does not infer structure from whitespace or OCR geometry; only repeated explicit TAB or pipe evidence already present in durable PAGE_TEXT may produce table evidence. STRONG/LIMITED OCR evidence becomes `TABLE_TEXT / LIMITED`, POOR evidence remains POOR, quality is never upgraded, and invalid PAGE_TEXT method/quality combinations fail closed. Multiple table occurrences remain distinct, cross-page segments are never stitched, and deterministic TABLE_TEXT ordinals use `pageCount + globalTableSourceOrder`, leaving existing PAGE_TEXT ordinals untouched. A reusable `DocumentNodePageLocator` preserves the deepest containing same-MaterialVersion node with document-root fallback and fails closed for duplicate nodes, missing/duplicate roots, cycles, missing parents, or cross-version hierarchy state. JDBC persistence independently verifies MaterialVersion eligibility, table ordinal/page boundaries, exact extraction method/quality, same-version DocumentNode identity, and that the durable DocumentNode page interval actually contains the TABLE_TEXT physical page. Persistence is parameterized, exact replay is a no-op preserving durable row identity, partial replay converges without duplicates, conflicting durable content/provenance fails closed, and finalization verifies the exact owned TABLE_TEXT count and ordinal range including the zero-table case. P3-04 extraction finalization was narrowly corrected to verify only `PAGE_TEXT` rows it owns, and regression coverage proves MATERIAL_EXTRACT replay succeeds after downstream TABLE_TEXT exists while preserving those rows unchanged. PDF/object-store work, temporary-file handling, PDFBox parsing, and page-geometry analysis execute outside database transactions; each durable table write and final verification uses a short transactional application boundary. Native table extraction is page-bounded and discards transient page geometry before advancing. Configured resource limits bound pages, native page characters, text positions, layout rows, rows/table, columns/table, tables/page, tables/document, and table output characters while structures are constructed rather than only after allocation. OCR delimiter scanning similarly enforces row/column/table/text budgets incrementally. Malformed, non-finite, rotated, overlapping, and off-crop-box geometry cannot establish confident table semantics. Initial focused implementation validation passed 44 tests with zero failures, errors, or skips. External general implementation review of initial head `050891565ca65c8f46a2e295566e4e82b717c432` identified four correction areas: (1) a three-column shortcut missed reliable two-column tables and insufficiently guarded columnar prose/side-by-side layouts; (2) several table resource and malformed-geometry budgets were enforced too late; (3) durable DocumentNode validation did not prove node/page containment; and (4) extractor page-count protocol and invalid PAGE_TEXT quality combinations were not fully fail-closed. Corrective commit `8b49a130ff53a6d227fbbae7a76debbc59fe2863` resolved all four findings. Correction-focused validation passed 52 tests with zero failures, errors, or skips, including `HippocampusArchitectureTests`, and `git diff --check` passed. External general implementation re-review of final head `7030527f6fb05cadebe7b19af928afaae69c0c2a` is **APPROVED** with no unresolved implementation blocker. Authoritative PR quality run [#328](https://github.com/KennethVier/hippocampus/actions/runs/34201772352) succeeded on that exact final head; backend-quality completed tests, architecture tests, migrations, and build successfully, and frontend-quality, auth-e2e, security scan, phase1-gate, and phase2-gate also succeeded. Independent adversarial security review verdict is **SECURITY PASS**, recorded on PR #126 as review ID `5139090396`, with 0 Critical, 0 High, and 0 Medium findings and no manual security review requirement for the reviewed P3-12 table-extraction/persistence surface. The security review covered owner-qualified processing, private object-storage boundaries, logical object-key/path safety, malicious PDF/resource exhaustion, incremental geometry/OCR parsing, transaction scope, SQL parameterization, same-version/node/page provenance, retry conflicts, temporary-file handling, and untrusted extracted text remaining inert data. PR #126 merged into `main` as `9ebc79b141d3466e2d5b3af6a5b1e338a8f64de2`. The Expected Result and Definition of Done are satisfied: trustworthy table structure is retained as searchable TABLE_TEXT when evidence is reliable; uncertain table evidence remains quality-limited without fabricated cell relations; extraction is page-bounded, retry-safe, provenance-checked, and resource-bounded; and the existing `VISUAL_EXTRACT -> NORMALIZE` processing sequence remains unchanged. No ADR or undocumented architectural deviation is required. No current blocker remains.
- **Notes / blockers:** _None_
...
- **Authority:** Documents 21
- **Evidence / link:** P3-13 was implemented in PR [#128](https://github.com/KennethVier/hippocampus/pull/128) at final reviewed head `d418fd52a4f96a3a038321fd97e3d154ffddf561` and merged into `main` as `6a15aef2b56561309893b7af7dd559e01e735129`. Accepted ADR-0004 and additive V12 preserve immutable raw extraction evidence in `content` and add nullable derived `normalized_content`. Native PAGE_TEXT normalization is deterministic and conservative, preserving medical notation while handling repeated page noise and high-confidence line-wrap/dehyphenation; OCR evidence is not lexically upgraded, blank PAGE_TEXT normalizes to `""`, and TABLE_TEXT is exact pass-through preserving TAB/LF. Bounded two-pass processing retains at most 32 candidate signatures, avoids whole-document source accumulation, and reuses existing typed native/OCR/table character limits. Persistence/finalization use short transactions, parameterized SQL, current Material/MaterialVersion eligibility locks, exact source-row provenance and same-version DocumentNode/page-containment checks, exact retry no-op, partial convergence, and fail-closed conflicts; finalization independently validates PAGE_TEXT/TABLE_TEXT durable state including zero tables. Real PostgreSQL regressions prove P3-04 PAGE_TEXT replay and P3-12 TABLE_TEXT replay preserve `normalized_content`, complete source identity/provenance, and table TAB/LF content. Complete focused validation passed 116 tests with 0 failures, 0 errors, 0 skips and BUILD SUCCESS; `git diff --check` passed. External general implementation review verdict is **APPROVED** with no unresolved implementation blocker. Independent adversarial security review verdict is **SECURITY PASS**, recorded on PR #128 as review ID `5145034401`, with 0 Critical, 0 High, and 0 Medium blocking findings and no manual security review requirement for this scoped P3-13 change. Authoritative exact-head PR quality run [#336](https://github.com/KennethVier/hippocampus/actions/runs/34256472952) (ID `34256472952`) succeeded on `d418fd52a4f96a3a038321fd97e3d154ffddf561`, with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, and `phase2-gate` successful. Successful NORMALIZE advances to CHUNK without implementing P3-14. The Expected Result and Definition of Done are satisfied, no undocumented architecture deviation is known, and no current blocker remains. P3-14 remains Not Started and Phase 3 remains unevaluated because P3-18 owns the phase gate.
- **Notes / blockers:** _None_
...
- **Evidence / link:** P3-14 hierarchy-aware chunking was implemented in PR [#131](https://github.com/KennethVier/hippocampus/pull/131), final reviewed head `d40290a826017460450979a6edf552b2534c8da0`, and merged into `main` as `14408c5a4c834701a7d2a7d3da630cf67d3e19e7`. Accepted ADR-0005 (PR #130) governs exact chunk-source provenance and deterministic `CHUNKER_V1` replay identity; the implementation uses the frozen UUIDv5 namespace `bc9da86d-4f4f-5f4f-9f59-8cc72e70b86a`. V13 adds durable chunks, exact ordered `chunk_text_block_links`, and visual links; chunking is hierarchy/method/content-type aware, bounded by configured target/hard-token and conservative overlap policy, consumes normalized PAGE_TEXT/TABLE_TEXT only, preserves exact primary-page visual provenance, and uses bounded page/persistence batches including the 601-page regression. PostgreSQL/Testcontainers coverage verifies deterministic replay and created-at preservation, compatible convergence, complete source-snapshot revalidation, source-position/overlap conflicts, exact-primary-page visual provenance, independent finalization including semantic-empty zero chunks, configured hard-token limits, and transaction rollback. External general implementation review verdict: **APPROVED**. Independent adversarial security review verdict: **SECURITY PASS**, with 0 Critical, 0 High, and 0 Medium findings on the exact final head. Authoritative exact-head quality run [#346](https://github.com/KennethVier/hippocampus/actions/runs/34360554903) (ID `34360554903`) succeeded on `d40290a826017460450979a6edf552b2534c8da0`; the complete merge candidate passed the required GitHub Actions validation. The Expected Result and Definition of Done are satisfied, no current blocker or undocumented architecture deviation remains, P3-15 remains Not Started, and Phase 3 remains unevaluated because P3-18 owns the phase gate.
- **Notes / blockers:** _None_
...
- **Evidence / link:** P3-15 processing recovery was implemented in PR [#133](https://github.com/KennethVier/hippocampus/pull/133) at final reviewed head `ba5daa37ab23d22b9e400321684d5c65da826ca5` and merged into `main` as `983e9354aef9ad2341df95ac00166d9e2c44f645`. The implementation adds PostgreSQL-authoritative claim/reclaim, heartbeat, current/total progress, worker+attempt fencing for execution mutations and successful stage completion, bounded retry/backoff, safe typed transient/fatal failure classification, stale final-attempt terminalization, restart discovery, and lock-skipping exhausted cleanup without weakening FIFO claim semantics. Recovery coverage proves due retries, strict stale detection, legacy NULL-heartbeat recovery, final-attempt exhaustion, concurrent lock-skipping cleanup, stale-worker fencing, replayed lower progress, restart persistence, recovery-enabled executor ownership-loss/failure handling, and a full transient-storage-failure → RETRY → reclaim → replay → completion lifecycle with exactly one durable output and no duplicate downstream stage. Authoritative exact-head quality run [#353](https://github.com/KennethVier/hippocampus/actions/runs/34426434858) (ID `34426434858`) succeeded on `ba5daa37ab23d22b9e400321684d5c65da826ca5`; `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, and `phase2-gate` all succeeded, and the PR `security` job completed explicit commit-range secret scanning and introduced-dependency review successfully. External general implementation review verdict: **APPROVED** with no unresolved implementation, architecture, concurrency, scope, or test blocker. Independent adversarial security review verdict: **SECURITY PASS** with 0 Critical, 0 High, and 0 Medium findings in the reviewed recovery surface; one LOW non-blocking hardening item remains to reject or preserve sub-second recovery durations before `Duration.toSeconds()` conversion. The Expected Result and Definition of Done are satisfied, no current blocker or undocumented architecture deviation remains, no ADR is required, and Phase 3 remains unevaluated because P3-18 owns the phase gate.
- **Notes / blockers:** Partial outcomes in P3-15 remain represented by the existing durable artifact-level quality/limitation metadata permitted by Document 21; they are not throwable job failures and no generic `PARTIAL` processing-job status is introduced. P3-16 remains responsible for deriving Material/MaterialVersion `READY`/`PARTIALLY_READY`/`FAILED` capability state. Initial `MATERIAL_VALIDATE` enqueue remains separately owned. Security hardening follow-up: sub-second recovery durations currently truncate through `Duration.toSeconds()`; defaults are safely above one second and this was assessed LOW/non-blocking.
...
- **Authority:** Documents 20,21
- **Evidence / link:** P3-17 was implemented in PR [#137](https://github.com/KennethVier/hippocampus/pull/137) at final reviewed head `98a5b474d6fb6edc398153084b98195359bd7a1f` and merged into `main` as `cf9101fdb964ba3f2c8688baf634a8780800b5cb`. The final implementation exposes separate owner-scoped processing-status and detected-structure reads, resolves the authoritative active/latest MaterialVersion server-side, projects bounded student-safe stage/readiness/limitation information without worker, retry, storage, database, or raw internal jargon, reads meaningful stage progress from durable `processing_jobs`, and gates hierarchy availability on successful durable `STRUCTURE_DETECT` completion so a pre-existing DOCUMENT root cannot expose an incomplete tree. The frontend keeps TanStack Query as server-state owner, polls only `UPLOADED`/`PROCESSING` using the reviewed 5s → 10s → 20s → 30s capped schedule, stops on terminal readiness, treats transient request failure separately from material failure, fetches the hierarchy separately only after structure becomes available, renders partial/failure limitations and indeterminate progress conservatively, and provides an accessible recursive collapsible hierarchy with non-disclosure leaf nodes. Required backend integration, RTL, and Playwright coverage exercises durable job progress/public-stage projection, owner/foreign fail-closed behavior, recursive hierarchy ordering and latest-version resolution, root-before-completion versus completed structure availability, readiness/partial/failure UI states, bounded polling/backoff/terminal behavior, transient request failure, hierarchy transition without remount, and tree accessibility. Authoritative exact-head quality run [#374](https://github.com/KennethVier/hippocampus/actions/runs/34545281638) (ID `34545281638`) succeeded on `98a5b474d6fb6edc398153084b98195359bd7a1f` with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, and `phase2-gate` all successful; `backend-quality` completed backend tests, architecture tests, PostgreSQL/Testcontainers migrations and build, `frontend-quality` completed lint/typecheck/tests/build/audit, and the PR `security` job passed explicit commit-range secret scanning and introduced-dependency review. External general implementation review verdict is **APPROVED** with no unresolved general-review blocker. Independent adversarial security review verdict is **SECURITY PASS** for the P3-17 changed surface, with no unresolved blocking Critical, High, or Medium finding. The Expected Result and Definition of Done are satisfied: students receive bounded understandable processing/usable-state/limitation feedback and the detected hierarchy without exposing internal worker/storage/DB details; required UX tests are green. No P3-18 or Phase 4 behavior, migration, provider/RAG capability, undocumented architecture deviation, or ADR requirement was introduced. No current blocker remains, and Phase 3 remains unevaluated because P3-18 owns the phase gate.
- **Notes / blockers:** _None_
...
**Phase Gate Evidence:** Phase 4 is complete with P4-01 through P4-14 `Done`. The phase established generation-scoped and rebuildable chunk embeddings; provider-independent embedding boundaries; authorized PostgreSQL lexical and pgvector vector retrieval; deterministic hybrid candidate merging; application-owned `RetrievalScope` construction and ownership/lifecycle filtering before ranking; inspectable, source-grounded `EvidencePackage`s; canonical `SourceReference` materialization and authenticated resolution; and bounded linked visual evidence without introducing generative AI as an authorization, retrieval, or educational-state authority. P4-12 established Golden Retrieval Dataset v1 as the committed deterministic retrieval-quality baseline, covering representative Anatomy and Physiology EXACT, SEMANTIC, and MIXED queries across lexical, vector, and hybrid channels with Recall@K/Precision@K at K=1/3/5, plain full-ranked-list MRR, expected-section hit rate, explicit irrelevant-context rate, committed baseline verification, and v1 baseline-as-threshold enforcement. P4-13 established the dedicated PostgreSQL/pgvector RAG isolation release suite and demonstrated zero known unauthorized chunks across semantic-identical User A/User B corpora, deliberately stronger foreign retrieval candidates, forged foreign-only and mixed scopes, foreign Topic access, deleted Materials, historical MaterialVersions, inactive Chunks, inactive Topic/link state, DocumentNode narrowing, forged foreign nodes, and empty-scope fail-closed behavior; its independent security review verified that production lexical/vector ownership and lifecycle predicates are applied before ranking. P4-14 then exercised the completed source-grounded pipeline as an integrated Phase 4 acceptance gate using representative Anatomy exact, Physiology semantic, and linked supported-visual contexts through authenticated `BuildRetrievalScope` → lexical and deterministic pgvector retrieval → `HybridCandidateMerger` → `BuildEvidencePackage` → canonical source-reference materialization → authenticated `ResolveSourceReference`, without Gemini, Ollama, chat generation, provider routing, prompts, LLM query expansion, LLM reranking, or any other generative-model dependency. Required local P4-14 validation passed `Phase4RetrievalGateIntegrationTests`, `GoldenRetrievalEvaluationIntegrationTests`, `SourceReferenceIntegrationTests`, `RagIsolationReleaseIntegrationTests`, and `HippocampusArchitectureTests`; `node scripts/validation/validate.mjs backend` returned **VERDICT PASS**, including broad backend `clean verify`, packaged backend verification, and repository-root `git diff --check`. External implementation review of P4-14 at exact head `895feeec6126cfc360e1b6de75b6c07f2a0134b0` returned **APPROVED** with no unresolved correctness, architecture, retrieval, provenance, authorization, Golden-baseline, CI-gate, or phase-scope blocker. Exact-head GitHub Actions quality workflow #480 (`35455390804`) completed **SUCCESS**, with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, `phase2-gate`, `phase3-gate`, and the new isolated `phase4-gate` all successful; `phase4-gate` requires all prior quality/phase gates to succeed before running `Phase4RetrievalGateIntegrationTests`. The security job scanned exact PR range `a7b9dcf0cea5b0c9c2bfaad426faa5b472dce87c..895feeec6126cfc360e1b6de75b6c07f2a0134b0` using pinned Gitleaks v8.28.0, scanning 1 commit / approximately 18.38 KB with **no leaks found**; dependency review found no introduced High-or-higher vulnerable package and no denied package. PR #172 was merged into `main` as `56c8f7a75317b5ad84996b29af42d1650dc2ead6`. Because PR #172 was merged before the mandatory independent security gate was executed, the independent `hippocampus-security-vulnerability-review` was completed immediately afterward against the exact unchanged implementation head `895feeec6126cfc360e1b6de75b6c07f2a0134b0` and returned **SECURITY PASS**, with no unresolved Critical, High, or Medium finding and no manual-security-review requirement; the review confirmed that P4-14 adds only integration-test, CI-gate, and tracker behavior, uses authenticated production retrieval/source-resolution boundaries, introduces no production authorization/retrieval/API/provider/frontend behavior, uses parameterized test persistence, adds no migration or dependency, performs no external AI call, preserves P4-13's zero-leakage gate, and does not weaken fail-closed retrieval or source-reference authorization. No implementation change occurred between the security-reviewed exact head and the merged implementation. The Document 26 Phase 4 completion criterion is therefore satisfied: given representative topic/query contexts, Hippocampus can reliably return correct authorized evidence before generative AI; authorization remains application-owned and occurs before ranking; retrieval quality remains measurable against the committed Golden baseline; zero cross-user retrieval leakage remains a release-gated invariant; resulting evidence is inspectable and traceable to canonical authorized sources; and the complete Phase 4 acceptance path is now durably enforced in CI. No current Phase 4 blocker or undocumented architectural deviation remains.
...
- **Evidence / link:** P4-02 was implemented in PR [#146](https://github.com/KennethVier/hippocampus/pull/146) at final reviewed head `559b0334d6217d7dd82ea563ef2d97156ddf33ed` and merged into `main` as `b802c7eca269c42595c496ca952567165fae5c5a`. The implementation adds the framework-independent `rag.port` embedding boundary: `EmbeddingPort`, immutable batch input/result and vector contracts, required provider/model/dimension metadata, optional model-version metadata, and provider-neutral input-token usage that explicitly distinguishes unavailable usage from a reported zero. Contract validation rejects null, empty, required blank metadata, supplied blank versions, negative reported usage, duplicate references, non-finite vectors, non-positive dimensions, and vector/model dimension mismatches while preserving defensive immutability. Three test-only fake adapters prove multi-input batching, caller-controlled UUID correlation, provider interchangeability without caller changes, provider-type containment, optional metadata handling, and dimension consistency without Spring, network, provider, persistence, or database dependencies. Focused validation passed 11 embedding contract tests and 9 ArchUnit tests with 0 failures, 0 errors, and 0 skips. Required backend validation passed with 771 tests, 0 failures, 0 errors, and 8 skips; `git diff --check` passed. Exact-head GitHub Actions quality run #421 (ID `34806538186`) completed successfully against `559b0334d6217d7dd82ea563ef2d97156ddf33ed`, including `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, `phase2-gate`, and `phase3-gate`. External implementation re-review verdict is **APPROVED FOR COMMIT AND PR PUBLICATION / PASS**, with the prior optional-metadata blocker resolved and no remaining correctness, architecture, testing, provider-boundary, or scope blocker. Independent security review verdict is **SECURITY PASS** for the P4-02 changed surface, with no unresolved blocking Critical, High, or Medium finding and no manual security review requirement. The Expected Result and Definition of Done are satisfied: chunk indexing now has a provider-independent embedding contract, no P4-03+ behavior or undocumented architecture deviation was introduced, and no current blocker remains.
- **Notes / blockers:** _None_
...
- **Evidence / link:** P4-03 implemented the initial direct Gemini embedding adapter behind the provider-independent `EmbeddingPort`. The backend imports the official Spring AI BOM at `2.0.1` and adds only `org.springframework.ai:spring-ai-google-genai-embedding`; the resolved AI path is Spring AI `2.0.1`, Google GenAI `1.65.0`, and the required Spring AI model/commons/template/retry transitive modules, with no chat, vector-store, Ollama, agent, memory, or provider-router dependency. `GeminiEmbeddingConfiguration` activates only when `hippocampus.rag.embedding.gemini.enabled=true`; normal startup remains credential-free, while enabled configuration fails closed for missing/blank `GEMINI_API_KEY`, blank model, or non-positive dimension. The configuration-owned candidate is `gemini-embedding-2` at dimension `768`. `GeminiEmbeddingAdapter` sends one unchanged text batch through Spring AI, correlates results by provider index to caller UUIDs, restores deterministic request order, validates exact cardinality, unique in-range indices, exact configured dimension, and finite immutable vectors, maps provider/model/version metadata, and preserves reported, zero, unavailable, and invalid usage semantics. Provider/client and malformed-response failures cross the port only as fixed-message provider-neutral failures without provider causes or sensitive payloads. Focused embedding/configuration/architecture validation passed 35 executed tests with 0 failures and 0 errors. The separately gated live smoke was then run with `GEMINI_API_KEY` set and `HIPPOCAMPUS_GEMINI_LIVE_TEST=true` using `.\mvnw.cmd -B -ntp "-Dtest=GeminiEmbeddingLiveSmokeTest" test`. The real Gemini smoke completed with **BUILD SUCCESS**: 1 test executed, 0 failures, 0 errors, 0 skips, test time 1.467 seconds, and total Maven time 4.234 seconds. It exercised the real `EmbeddingPort` path and verified caller correlation, model metadata, and configured 768-dimensional finite vectors. Required `node scripts/validation/validate.mjs backend` passed clean backend verification with 787 tests, 0 failures, 0 errors, and 9 skips; dependency review, architecture review, scope inspection, and `git diff --check` also passed. No P4-04 job/orchestration, persistence/upsert, repository, migration, retrieval, endpoint, frontend, prompt policy, multimodal, Ollama, chat, or later-phase behavior was introduced. Final validation verdict was **VALIDATION PASS**. External implementation review returned **APPROVED** at reviewed head `5099d35c45650e3d8ae0d2e36c29f2e8548aaab5`. Exact-head quality run #428 (`34812328875`) completed successfully, including `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, and applicable phase gates. The security job scanned the complete PR commit range and reported no leaked secrets; dependency review found no blocking introduced dependency vulnerability. The mandatory independent `hippocampus-security-vulnerability-review` returned **SECURITY PASS**, with no unresolved blocking Critical, High, or Medium security finding within the reviewed P4-03 scope. Implementation PR #149, `P4-03: Add Gemini embedding adapter`, was merged on 2026-09-14 with merge commit `f865ae18d279c70d9fa7327720349ecc26204744`. All required implementation, validation, external review, exact-head CI, security, and merge completion gates are satisfied.
- **Notes / blockers:** *None*
...
- **Authority:** Documents 21
- **Evidence / link:** P4-04 implemented the durable provider-neutral `EMBED` processing stage in implementation PR [#151](https://github.com/KennethVier/hippocampus/pull/151). `EmbedMaterialVersion` reads only active chunks for one non-deleted material version in deterministic `chunk_index` order, submits bounded batches of unchanged canonical content through `EmbeddingPort` with chunk UUID correlation, verifies provider cardinality/reference/model/dimension compatibility, checks processing-job ownership before and after each provider call, and reports durable embedded/eligible counts. Provider calls execute outside database transactions; successful batches are persisted through the short transactional `PersistEmbeddingBatch` collaborator and `JdbcEmbeddingJobRepository`. PostgreSQL persistence uses generation-scoped `ON CONFLICT (chunk_id,index_generation_id) DO NOTHING`, locks and revalidates the owning material/version as non-deleted before writing, validates that every chunk is active and belongs to the requested material version, preserves existing valid rows on replay, and retains the V15 pgvector dimension invariant. Generation selection prefers exactly one `ACTIVE` generation, otherwise exactly one `BUILDING` generation, fails closed on ambiguity or incompatible provider/model/version/dimension metadata, and concurrency-safe initial generation creation records actual first-response metadata with `CHUNKER_V1`. Processing progression is handler-availability based: `CHUNK` durably advances to `EMBED` only when the handler exists, while `EMBED` stops before unavailable `INDEX`; no speculative P4-05+ indexing/retrieval behavior was introduced. Recovery classification maps provider failures to transient `EMBEDDING_PROVIDER_UNAVAILABLE` and invalid provider responses to fatal `EMBEDDING_RESPONSE_INVALID`. Deterministic validation proved bounded 7-at-3 batching (`3/3/1`), canonical input/correlation, durable progress, inactive-chunk exclusion, deletion fencing before work, between batches, and between provider return and persistence, provider calls outside transactions, partial-success durability followed by retry of only missing chunks, zero-call replay idempotency, malformed/mismatched response rejection, ownership-loss fencing, concurrent initial-generation creation, real pgvector persistence and dimension enforcement, generation-scoped uniqueness, ambiguous-generation fail-closed behavior, mixed-material batch rejection with complete rollback, and real `CHUNK -> EMBED` completion without speculative `INDEX`. Focused validation passed 19 tests: 7 unit tests and 12 PostgreSQL/Testcontainers tests, with 0 failures, 0 errors, and 0 skips. Required `node scripts/validation/validate.mjs backend` passed with 809 tests, 0 failures, 0 errors, and 9 skips; `git diff --check` passed. No schema migration or new dependency was introduced. External implementation review returned **APPROVED** at final reviewed head `2cc54e58b498b6c42ae4141233f1f3f30fa474f8`, with no unresolved correctness, concurrency, transaction-boundary, idempotency, architecture, processing-stage, testing, or P4-04 scope blocker. Exact-head GitHub Actions quality run #433 (`34833262229`) completed successfully against that reviewed head, including `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, `phase2-gate`, and `phase3-gate`. The independent `hippocampus-security-vulnerability-review` returned **SECURITY PASS** with no unresolved blocking Critical, High, or Medium finding within the reviewed P4-04 surface; the review specifically verified job/material ownership boundaries, deletion-race fencing, provider-response validation, SQL/vector persistence integrity, replay/idempotency protections, secret handling, exceptional-condition fail-closed behavior, and the unchanged dependency surface. Implementation PR #151 was merged into `main` on 2026-09-14 with merge commit `e0bef69cd49e941e712c60b23fd7b54454dfa838`. All required implementation, validation, external review, exact-head CI, independent security, merge, Expected Result, and Definition of Done gates are satisfied. No undocumented architecture deviation or P4-05+ behavior remains.
- **Notes / blockers:** *None*
...
- **Authority:** Documents 13,17
- **Evidence / link:** P4-05 implemented the provider-neutral PostgreSQL lexical search repository in implementation PR [#153](https://github.com/KennethVier/hippocampus/pull/153). `JdbcLexicalSearchRepository` performs candidate retrieval using PostgreSQL Full-Text Search with `to_tsvector('simple', content)` and `websearch_to_tsquery('simple', ...)`, case-insensitive literal matching with escaped `LIKE` metacharacters, and `pg_trgm` word similarity. The repository preserves canonical chunk content and provenance and exposes independent lexical ranking signals without introducing vector search, hybrid rank fusion, EvidencePackage construction, or LLM behavior. Authorization and retrieval eligibility are enforced inside the same PostgreSQL candidate query: chunks must be active, the owning material must belong to the requested user and not be deleted, the candidate material version must equal the material's active version and be explicitly included in the allowed material-version scope, and optional document-node restrictions are applied in SQL. Empty allowed-version scope fails closed. V16 adds partial GIN indexes for `to_tsvector('simple', content)` and `content gin_trgm_ops` over active chunks while reusing the existing `pg_trgm` extension; no new dependency or external search infrastructure was introduced. Deterministic PostgreSQL/Testcontainers coverage proved golden medical lexical retrieval for `C5-T1`, `CN VII`, `β1`, `Na+`, `IL-6`, and `posterior cord`, ordinary FTS behavior, case-insensitive retrieval, trigram typo matching, exact-before-fuzzy ordering, cross-user isolation, rejection of deliberately supplied foreign-version UUIDs, historical/non-active-version exclusion, inactive-chunk exclusion, deleted-material exclusion, document-node narrowing, empty-scope fail-closed behavior, literal `%`, `_`, and backslash escaping, SQL-injection-shaped input remaining data, caller result limits, deterministic tie ordering, provenance preservation, and installation of the V16 lexical indexes. Required `node scripts/validation/validate.mjs backend` returned **VALIDATION PASS** and `git diff --check` passed. External implementation review returned **APPROVED** at final reviewed head `49798a32fb9da3b0daf5029528791fabf3d5d7da`, with no unresolved correctness, SQL-authorization, cross-user-isolation, active-version, parameterization, ranking/indexing, provenance, architecture, testing, or P4-05 scope blocker. Exact-head GitHub Actions quality run #437 (`34843991793`) completed successfully against that reviewed head, including `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, and applicable phase gates. The CI security job scanned the complete two-commit PR range with Gitleaks and reported no leaked secrets; dependency review reported no introduced High-or-higher vulnerable package and P4-05 introduced no dependency changes. The mandatory independent `hippocampus-security-vulnerability-review` returned **SECURITY PASS**, with no unresolved blocking Critical, High, or Medium finding within the reviewed P4-05 surface. The independent review specifically verified authorization-before-ranking, cross-user and malicious-scope isolation, active/deleted/historical-material fencing, parameterized SQL, literal wildcard escaping, fail-closed empty scope, untrusted medical text handling, provenance integrity, exceptional-condition behavior, migration safety, secrets, and supply-chain impact. Implementation PR #153, `P4-05: Add lexical search repository`, was merged into `main` on 2026-09-14 with merge commit `976615cad28dd4d15de403fc7087871d7db30e8b`. All required implementation, Expected Result, Definition of Done, validation, external review, exact-head CI, independent security, and merge completion gates are satisfied. No undocumented architectural deviation or P4-06+ behavior remains.
- **Notes / blockers:** *None*
...
- **Evidence / link:** P4-06 implemented the provider-neutral vector search repository in implementation PR [#155](https://github.com/KennethVier/hippocampus/pull/155). `VectorSearchRepository`, `VectorSearchScope`, `VectorSearchRequest`, and source-traceable `VectorSearchHit` contracts were added with Spring-composed `JdbcVectorSearchRepository`. Exact PostgreSQL/pgvector cosine retrieval uses a `MATERIALIZED` authorized-candidate CTE so authorization and eligibility are enforced before distance calculation or ranking. Candidate predicates require the requested index generation, `ACTIVE` generation status, matching embedding dimension, active chunk, owning user, non-deleted material, candidate material version equal to the material's active version, explicit allowed-version membership, optional allowed-document-node membership, and positive stored embedding vector norm. Empty allowed-version scope fails closed; all-zero query vectors are rejected; zero-norm stored embeddings are excluded before ranking; missing, `BUILDING`, `INACTIVE`, `FAILED`, or dimension-incompatible generations return no candidates. Retrieval uses pgvector cosine distance with finite `1 - cosine_distance` similarity values and deterministic ordering by distance, chunk index, then chunk UUID while preserving canonical content and source provenance. PostgreSQL 18 + pgvector Testcontainers validation passed 10 vector repository tests covering known-vector cosine ranking, result limits, provenance, node narrowing, historical/deleted/inactive exclusion, generation isolation, dimension mismatch, deterministic ties, mixed and zero-only zero-norm candidate exclusion, and zero cross-user leakage. The User A/B security case proved that User B's semantically identical and mathematically perfect vectors remain excluded from User A's results even when User B's material-version UUID is deliberately included in the supplied scope; a forged foreign-only scope returned zero rows. Seven focused request/hit tests and ten architecture tests passed. Required `node scripts/validation/validate.mjs backend` returned **VALIDATION PASS** with 841 tests, 0 failures, 0 errors, and 9 skips; `git diff --check` passed. External implementation review initially identified the zero-norm stored-vector edge and, after correction at final reviewed head `fc2453a54752afe7df65877c93c471e5d06ae3b9`, returned **APPROVED** with no remaining correctness, authorization, generation, pgvector, provenance, architecture, testing, or P4-06 scope blocker. Exact-head GitHub Actions quality run #442 (`34858555012`) completed successfully, including `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, `phase2-gate`, and `phase3-gate`. The exact-head security job scanned the full two-commit PR range with Gitleaks and reported no leaked secrets; dependency review reported no introduced High-or-higher vulnerable package or denied dependency, and P4-06 introduced no dependency changes. The mandatory independent `hippocampus-security-vulnerability-review` returned **SECURITY PASS**, with no unresolved blocking Critical, High, or Medium finding in the reviewed P4-06 scope. The security review specifically verified authorization before vector ranking, cross-user and forged-scope isolation, active/deleted/historical-material fencing, generation status and dimension isolation, parameterized SQL, query and stored zero-vector handling, finite similarity enforcement, provenance integrity, exceptional-condition fail-closed behavior, secrets, configuration, and supply-chain impact. Implementation PR #155, `P4-06: Add vector search repository`, was merged into `main` on 2026-09-14 with merge commit `eac9804d495f9343327f90210ccb1f33bcdc27db`. All required implementation, Expected Result, Definition of Done, validation, external review, exact-head CI, independent security, and merge completion gates are satisfied. No migration, new dependency, ANN index, provider call, controller, frontend change, undocumented architectural deviation, or P4-07+ behavior remains.
- **Notes / blockers:** *None*
...
- **Evidence / link:** P4-07 implemented deterministic, provider-neutral hybrid candidate fusion in implementation PR [#157](https://github.com/KennethVier/hippocampus/pull/157). `HybridCandidateMerger`, `HybridCandidate`, `HybridFusionPolicy`, `HybridLexicalSignal`, and `HybridVectorSignal` were added in `com.hippocampus.rag.application` over already-authorized and already-ranked `LexicalSearchHit` and `VectorSearchHit` inputs. The balanced Reciprocal Rank Fusion policy uses `fusionScore = 1.0 / (60 + lexicalRank) + 1.0 / (60 + vectorRank)`, with missing-channel contribution zero; configurable weights must be finite and non-negative, at least one weight must be positive, and `rrfK >= 1`. Raw FTS, trigram, exact-match, and cosine values remain diagnostics rather than directly comparable fusion inputs. Candidates deduplicate by `chunkId`; duplicate IDs within either input channel fail closed, and a shared chunk must have identical canonical material, version, node, chunk, content, page, heading, content-type, extraction-method, and quality provenance across both channels or merging fails with `IllegalStateException`. Final ordering is fusion score descending then chunk UUID ascending before the caller limit, with immutable results and defensively copied heading provenance. The P4-07 synthetic offline baseline, intentionally separate from the later P4-12 formal Golden Retrieval Dataset, passed exact lexical (`C5-T1`), semantic-only (`wrist drop`), and mixed lexical/semantic (`β1 receptor` / `SA node automaticity`) fixtures. Every fixture achieved Recall@3 = `1.0` and MRR = `1.0`; the mixed rank-2/rank-2 dual-channel candidate ranked ahead of both rank-1 single-channel competitors, demonstrating complementary lexical and semantic retrieval behavior. Focused validation passed 36 P4-07 tests, including the three offline metric cases, and all 10 architecture tests passed. Required `node scripts/validation/validate.mjs backend` returned **VALIDATION PASS** with 877 tests, 0 failures, 0 errors, and 9 skips; `git diff --check` passed. External implementation review at final head `dabf1b101d9df5419aebbfd427f7a34d3c083cbb` returned **APPROVED** with no unresolved correctness, architecture, scope, fusion, provenance-integrity, testing, or security-readiness issue. The mandatory independent `hippocampus-security-vulnerability-review` returned **SECURITY PASS**, with no unresolved blocking Critical, High, or Medium finding. The review verified that P4-07 cannot widen authorization scope because it performs no retrieval or persistence operation, that authorization remains enforced by the upstream lexical/vector repositories before ranking, that provenance disagreement and duplicate ambiguity fail closed, that invalid numeric/policy states cannot silently propagate, and that no SQL, command, path, rendering, provider, prompt, API, session, upload, or external trust boundary was introduced. Exact-head GitHub Actions quality run #446 (`34866681355`) completed successfully, including `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, `phase2-gate`, and `phase3-gate`. The exact-head security job scanned the PR commit range with Gitleaks and reported no leaked secrets; dependency review reported no introduced High-or-higher vulnerable or denied package, and P4-07 added no dependency. Implementation PR #157 was merged into `main` with merge commit `a9a82905a00e38c9af58cea96a8a9e0bcb4205b6`. All implementation, Expected Result, Definition of Done, validation, external review, exact-head CI, independent security review, and merge gates are satisfied. No repository orchestration, centralized `RetrievalScope`, `EvidencePackage`, `SourceReference`, retrieval inspector, formal golden dataset, model/metadata reranking, LLM behavior, API/controller, frontend, migration, dependency, or other P4-08+ behavior was introduced.
- **Notes / blockers:** *None*
...
- **Dependencies:** Retrieval repositories and material/topic relationships
- **Tests / validation:** Cross-user and inactive-version scope tests.
- **Expected result:** Every retrieval is scoped centrally.
- **Definition of Done:** Scope tests pass.
- **Authority:** Documents 13,22
- **Evidence / link:** P4-08 implemented the centralized, server-owned RAG retrieval authorization boundary in implementation PR [#159](https://github.com/KennethVier/hippocampus/pull/159). Provider/framework-neutral `GroundingMode`, `RetrievalScope`, and `RetrievalScopeTarget` domain contracts were added. `BuildRetrievalScope.Query` accepts only `topicId` and `GroundingMode`; authenticated ownership is derived exclusively from `CurrentUser.authenticatedUser().userId()`, so callers cannot supply or widen authorization through user, Material, MaterialVersion, or DocumentNode identifiers. `JdbcRetrievalScopeSourceRepository` resolves eligible scope rows using bound parameters and requires an ACTIVE requested Topic under an ACTIVE Subject owned by the authenticated user, an ACTIVE `MaterialTopicLink`, a same-user non-DELETED Material, and a non-null current `materials.active_version_id`. Material-level links with a null explicit version resolve to the current active version; explicit MaterialVersion links are eligible only when equal to that active version, excluding historical versions. Optional DocumentNode links must resolve against the same current active MaterialVersion. Node rows coalesce by MaterialVersion, while any whole-version authorization for that version dominates narrower node rows. `GENERAL_KNOWLEDGE`, `SOURCE_FIRST`, and `STRICT_SOURCE` preserve identical ownership scope and cannot widen authorization. Flyway V17 (`V17__enable_material_topic_link_document_nodes.sql`) removed the obsolete Phase-2 node-disable constraint while retaining the node-requires-version rule and adding the restrictive composite `(document_node_id, material_version_id) -> document_nodes(id, material_version_id)` foreign key. The former duplicated `LexicalSearchScope` and `VectorSearchScope` contracts were removed and lexical/vector requests now consume the central `RetrievalScope`. Both search repositories retain independent defense-in-depth checks for owning user, non-DELETED Material, active Chunk, current MaterialVersion, and explicit scope membership; vector retrieval additionally preserves ACTIVE generation, embedding-dimension, and zero-norm protections. Whole-version-only, node-only, mixed, and empty scopes are supported without invalid empty `IN ()` SQL. Mixed-scope regression tests proved that all eligible chunks from a whole Version A remain searchable while Version B is restricted to its specifically authorized node, and a mathematically better unauthorized vector candidate is excluded before cosine ranking. Cross-user, foreign Topic, malicious cross-user link, archived Subject/Topic, inactive link, deleted Material, missing active version, historical version, malformed node, and mismatched node/version cases fail closed. Focused P4-08 domain/application/request, resolver, search, Flyway, and architecture validation passed 56 tests, followed by 7 updated MaterialTopicLink regression tests; all 10 architecture tests passed. Required `node scripts/validation/validate.mjs backend` returned **VALIDATION PASS** with 884 tests, 0 failures, 0 errors, and 9 skips; `git diff --check` passed. External implementation review at final reviewed head `fa5ce47d9b287320f94259cebf43511dfcdc1cba` returned **APPROVED** with no unresolved correctness, architecture, authorization, migration, testing, or P4-08 scope blocker. The mandatory independent `hippocampus-security-vulnerability-review` subsequently returned **SECURITY PASS**, with no unresolved blocking Critical, High, or Medium finding. The security review verified BOLA/cross-user isolation, `CurrentUser` ownership derivation, active Topic/Subject/Material/version fencing, client scope-widening resistance, node/version integrity, authorization-before-ranking, GroundingMode non-widening, parameterized SQL, fail-closed exceptional behavior, V17 migration integrity, secrets, configuration, and supply-chain impact. Exact-head GitHub Actions quality run #450 (`34934485909`) completed successfully with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, `phase2-gate`, and `phase3-gate` all green. Gitleaks scanned the explicit implementation commit range and found no leaked secrets; dependency review found no introduced High-or-higher vulnerable or denied package, and P4-08 introduced no dependency. Implementation PR #159 was merged into `main` on 2026-09-15 with merge commit `c72411e21ebdface1719e0a77534fdd74d00b230`. All required implementation, Expected Result, Definition of Done, validation, external review, independent security review, exact-head CI, and merge completion gates are satisfied. No P4-09 `EvidencePackage`, SourceReference resolution, retrieval inspector, formal golden dataset, retrieval/embedding orchestration, LLM/provider behavior, controller/API, frontend behavior, ranking change, external service, dependency, or undocumented architectural deviation was introduced.
- **Notes / blockers:** *None*
...
- **Evidence / link:** P4-09 implemented the normalized, bounded, source-traceable RAG evidence boundary in implementation PR [#161](https://github.com/KennethVier/hippocampus/pull/161). The implementation introduced immutable provider/framework-neutral `EvidencePackage`, `EvidenceChunk`, `EvidenceVisual`, package-local `EvidenceSourceReference`, `EvidenceReferenceKind`, `EvidencePackageBudget`, typed `EvidenceLimitation`/`EvidenceLimitationCode`, qualitative `RetrievalQuality`, and factual `RetrievalDiagnostics` contracts. `BuildEvidencePackage.Command` accepts only the server-owned `RetrievalScope`, already-ranked `HybridCandidate` values, explicit retrieval quality, and server-selected chunk/visual budget. It preserves hybrid ranking order and canonical source text verbatim, rejects duplicate chunk candidates, and revalidates every candidate with centralized `RetrievalScope.allows(...)` before packaging. Whole-version and node-restricted authorization semantics remain centralized rather than being reimplemented in another scope type. `JdbcEvidenceVisualRepository` uses parameterized SQL and fixed application-owned authorization clauses over `chunk_visual_links`, active `chunks`, `material_versions`, owning/non-DELETED/current `materials`, and same-version `visual_assets`. It independently checks both selected-chunk authorization and visual authorization, so an authorized node-scoped chunk cannot implicitly authorize a linked visual from another node or a null-node visual; whole-version and mixed scopes preserve their intended breadth. SUPPORTED visuals are included, LIMITED visuals are included with `LIMITED_VISUAL_INTERPRETATION`, and UNASSESSED/UNSUPPORTED/FAILED visuals are excluded with `VISUAL_EXCLUDED_BY_INTERPRETATION_STATUS`. Multi-chunk visual associations deduplicate to one `EvidenceVisual`, union linked chunk IDs and relationship types, and fail closed on conflicting visual provenance or metadata. Evidence is bounded using explicit `maxChunks`/`maxVisuals`; truncation produces deterministic `CHUNK_BUDGET_APPLIED` and `VISUAL_BUDGET_APPLIED` limitations rather than introducing a universal retrieval top-K. Retrieval quality is intentionally separate from source extraction/OCR quality: STRONG/LIMITED require selected chunk evidence, INSUFFICIENT may be empty and records `INSUFFICIENT_EVIDENCE`, while FAILED rejects candidates, skips visual retrieval, emits no evidence/source references, and records `RETRIEVAL_FAILED`. Additional deterministic limitations cover limited/poor chunk source quality and limited/excluded visual interpretation. Package-local CHUNK/VISUAL `EvidenceSourceReference` records preserve machine provenance without implementing P4-10 persistence/resolution; `EvidencePackage` validates that every chunk and visual has exactly one matching reference, rejects duplicate/dangling/fabricated references, validates material/version/node/page provenance agreement, validates exact diagnostics/evidence consistency, and prevents FAILED packages from carrying trusted evidence. `RetrievalDiagnostics` records candidate count, selected chunk count/IDs, selected visual IDs, actual source material IDs, selected vector index-generation IDs, and retrieval quality without fabricating unavailable latency/model/token metadata. Focused validation passed 24 P4-09 domain/application/PostgreSQL tests, 3 RetrievalScope regression tests, and all 10 architecture tests (37 focused tests total). Required backend validation returned **VALIDATION PASS** with 908 tests, 0 failures, 0 errors, and 9 skips; `git diff --check` passed. External implementation review at final implementation head `880f0aa55742ec8ace5d9db38077a84f083775c8` returned **APPROVED** with no unresolved correctness, architecture, authorization, evidence-integrity, validation, or P4-09 scope blocker. The mandatory independent adversarial `hippocampus-security-vulnerability-review` subsequently returned **SECURITY PASS**, with no unresolved blocking Critical, High, or Medium finding. Security review verified candidate scope revalidation, cross-user/current-version/active-source fencing, independent visual-node authorization, package-local source-reference integrity, parameterized SQL, prompt-like source content remaining inert data, absence of private storage-key exposure/source-text logging, and fail-closed exceptional behavior. Exact-head GitHub Actions quality run #454 (`34946448283`) completed successfully with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, `phase2-gate`, and `phase3-gate` all green. The security job scanned the exact PR range with pinned Gitleaks and reported no leaked secrets; dependency review reported no introduced High-or-higher vulnerable or denied package, and P4-09 added no dependency. Implementation PR #161 was merged into `main` with merge commit `7407d1acd2c276291f1a1853d13cc74cd86e2fea`. No Flyway migration, canonical/persisted SourceReference resolver, stable citation ID, learner-facing display label, reference lookup/opening flow, retrieval inspector, Golden Retrieval Dataset, full RAG isolation gate, ContextAssembler, `SOURCE_CONTEXT`, prompt/provider behavior, controller/API, frontend, ranking change, dependency, or other P4-10+ behavior was introduced. All P4-09 implementation, Expected Result, Definition of Done, validation, external review, independent security review, exact-head CI, and merge gates are satisfied.
- **Notes / blockers:** *None*
...
- **Dependencies:** EvidencePackage
- **Tests / validation:** SourceReference security tests.
- **Expected result:** Valid references open correct source.
- **Definition of Done:** Security tests pass.
- **Authority:** Documents 18,22
- **Evidence / link:** P4-10 implemented materials-owned canonical `SourceReference` persistence and authorization-safe source resolution in implementation PR [#163](https://github.com/KennethVier/hippocampus/pull/163). Flyway `V18__create_source_references.sql` introduced the authoritative SourceReference schema with Material/MaterialVersion, optional DocumentNode/Chunk/Visual/Page/timestamp targets, deterministic learner-facing display labels, restrictive same-version composite foreign keys, meaningful-target/page/timestamp/range checks, chunk-versus-visual exclusivity, canonical `UNIQUE NULLS NOT DISTINCT` identity, and justified target-resolution indexes. Canonical SourceReference ownership remains in `materials`; `SourceReference`, sealed page/node/chunk/visual target contracts, `CreateSourceReferences`, `ResolveSourceReference`, `SourceReferenceRepository`, `SourceReferenceSeed`, and `JdbcSourceReferenceRepository` remain materials-owned, while an explicit architecture rule prevents `materials -> rag` dependency. Both creation and resolution derive the authoritative user exclusively through `CurrentUser.authenticatedUser().userId()` and do not accept caller-controlled user identity. Creation requires the owning non-DELETED Material, exact current active MaterialVersion, and valid target provenance; active Chunk state is required where applicable. Node/page provenance for Chunk and Visual targets is derived from authoritative PostgreSQL state rather than caller-supplied redundant metadata. Single-page chunks retain their canonical page; multi-page chunks deliberately store no fabricated page. Creation is transactional, rejects duplicate primary targets, preserves input order, locks authoritative source rows during creation, and uses concurrency-safe canonical upsert so repeated registration returns the same SourceReference UUID and `createdAt` while allowing deterministic labels to refresh from current authoritative metadata. Labels are plain text generated from authoritative Material title, optional page, and nonblank DocumentNode title; they contain no internal UUID and use no LLM. `ResolveSourceReference` accepts only the SourceReference UUID, re-derives the authenticated user, and resolves only while the Material remains owned/non-DELETED, the referenced MaterialVersion remains the Material's active version, and referenced Chunk/Node/Visual/Page targets remain valid; Chunk references additionally require the Chunk to remain active. Random/fabricated IDs, foreign-user references, deleted Materials, historical versions, inactive Chunks, missing targets, and malformed/inconsistent references converge on the same sanitized `SOURCE_REFERENCE_NOT_FOUND` failure, preventing the SourceReference ID from becoming an existence/authorization oracle. Resolver output contains canonical source identity/provenance only and exposes no storage key, filesystem path, source binary, raw source text, signed URL, or permanent object URL. `MaterializeEvidenceSourceReferences` provides the narrow P4-09 bridge, mapping package-local CHUNK/VISUAL `EvidenceSourceReference` values into materials-owned targets in package order without mutating the `EvidencePackage`; canonical authorization is rechecked during materialization and repeated materialization returns stable SourceReference IDs. Focused P4-10/P4-09 regression/architecture validation passed 48 tests with 0 failures/errors, including 10 PostgreSQL/Testcontainers SourceReference scenarios and all 11 architecture tests. Full-history Flyway validation successfully applied all 18 migrations from an empty schema and repeated cleanly. Required backend validation returned **VALIDATION PASS** with 922 tests, 0 failures, 0 errors, and 9 skips; packaged backend build and `git diff --check` passed. External implementation review at final implementation head `8674805ccd8babbf1237b8694fdff73576eb2295` returned **APPROVED** with no unresolved correctness, architecture, authorization, persistence, migration, validation, or scope blocker. Mandatory independent adversarial security review at the same exact head returned **SECURITY PASS**, with no unresolved blocking Critical, High, or Medium finding. The review verified BOLA/IDOR resistance, server-owned CurrentUser identity, uniform foreign/missing-reference behavior, current-version/deleted/inactive fencing, canonical provenance integrity, transactional batch rollback, concurrency-safe stable IDs, parameterized SQL, no private-source exposure, and fail-closed stale-reference behavior. Exact-head GitHub Actions quality run #458 (`34987234227`) completed successfully with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, `phase2-gate`, and `phase3-gate` all green. The exact PR range `e181159207a04859c78f4b75c878f047e1d8527b..8674805ccd8babbf1237b8694fdff73576eb2295` was scanned using pinned Gitleaks v8.28.0: one commit / approximately 59.86 KB scanned, no leaked secrets found. Dependency review found no introduced High-or-higher vulnerable package and no denied package; P4-10 added no external dependency. Implementation PR #163 was merged into `main` with merge commit `d96bae41fd09e1bb0c594d6bfa0462463ad424c4`. No controller/API, source-file streaming, signed URL generation, object-storage delivery, P4-11 retrieval inspector, P4-12 Golden Retrieval Dataset, P4-13 full RAG isolation suite, retrieval orchestration, ContextAssembler, `SOURCE_CONTEXT`, prompt/provider behavior, frontend behavior, timestamp navigation behavior, or other P4-11+ scope was introduced. All P4-10 implementation, Expected Result, Definition of Done, validation, external review, independent security review, exact-head CI, and merge gates are satisfied.
- **Notes / blockers:** *None*
...
- **Evidence / link:** P4-11 implemented the protected developer retrieval inspector at `POST /api/dev/rag/inspect` in implementation PR #165. The inspector is available only when the `local` Spring profile is active and `hippocampus.rag.inspector.enabled=true`; only `application-local.yml` enables the feature, while default and pilot profiles keep the inspector absent. The endpoint remains inside the existing authenticated `/api/**` session boundary and normal CSRF protection, with tests proving unauthenticated requests return `401 AUTHENTICATION_REQUIRED`, authenticated requests without CSRF return `403 CSRF_VALIDATION_FAILED`, and authenticated requests with valid CSRF reach the inspector. The request contract accepts only `topicId`, exact unmodified `query`, and `groundingMode`; it accepts no caller-owned user ID, Material/MaterialVersion/DocumentNode scope, index-generation ID, query vector, fusion weight, diagnostic limit, or arbitrary RetrievalScope. `InspectRetrieval` derives authorization first through `BuildRetrievalScope` and `CurrentUser`. Empty authorized scope short-circuits lexical search, ACTIVE-generation lookup, embedding-provider invocation, vector search, and hybrid retrieval. Lexical and vector repositories retain authorization defense in depth through authenticated-user ownership, active MaterialVersion, non-DELETED Material, active Chunk, and authorized version/node predicates before ranking. Diagnostic results expose only privacy-safe metadata including authorized scope, candidate IDs, provenance, heading paths, page ranges, quality classifications, ranks, lexical/vector/hybrid scores, index-generation metadata, candidate counts, and total duration. Existing retrieval/merge objects may internally carry source content, but source content is never copied into `RetrievalInspection` or the HTTP response. The response additionally excludes authenticated user ID, storage keys, signed URLs, source binaries, filesystem paths, prompts, student answers, provider payloads, and provider exception details. `ActiveIndexGenerationRepository` and `JdbcActiveIndexGenerationRepository` read only `ACTIVE` index generations, return empty when none exist, return the single generation when exactly one exists, and fail closed if multiple ACTIVE generations are present; no BUILDING-generation fallback is reused. Vector diagnostics remain optional when no `EmbeddingPort` or ACTIVE generation exists. Query embedding uses one unchanged query with a fresh correlation UUID and validates result count, correlation, model/provider/version metadata, vector dimension, and non-zero usability before vector retrieval. Known provider failures map to fixed diagnostic states without exposing provider errors. Existing `HybridCandidateMerger` and fusion policy are reused unchanged, supporting lexical-only, vector-only, and combined diagnostics without introducing new ranking policy. Focused P4-11 validation passed 28 tests, architecture validation passed all 11 architecture tests, and the profile-start regression passed 3 tests. Full backend validation returned **VALIDATION PASS** with 937 tests, 0 failures, 0 errors, and 9 skipped; `git diff --check` passed. The production-composed PostgreSQL/Testcontainers smoke used a synthetic `posterior cord` query and demonstrated the authorized chunk ID, `Posterior Cord` heading, page 12, STRONG quality, lexical/vector/hybrid ranking signals, and active generation while excluding an otherwise stronger foreign-user candidate, the foreign candidate UUID, and both private source-text sentinels from the serialized response. External implementation review at final implementation head `35d01fd920dba479523e30c5edc4b04f5e6a9432` returned **APPROVED** with no unresolved correctness, architecture, authorization, privacy, retrieval, configuration, or scope blocker. The mandatory independent adversarial security review at the same exact head returned **SECURITY PASS**, with no unresolved blocking Critical, High, or Medium finding. The review verified BOLA/IDOR resistance, server-owned scope, authorization-before-ranking, authenticated session and CSRF enforcement, local-only configuration, cross-user retrieval isolation, privacy-safe diagnostic projection, SQL parameterization, provider/error sanitization, fail-closed ACTIVE-generation and embedding handling, secret scanning, and dependency/supply-chain impact. Exact-head GitHub Actions quality run #463 (`35058790116`) completed successfully with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, `phase2-gate`, and `phase3-gate` all green. CI scanned exact PR range `d087b48f33aba3a45875c45dcfe1f1d3db1c31d6..35d01fd920dba479523e30c5edc4b04f5e6a9432` using pinned Gitleaks v8.28.0: 2 commits / approximately 65.63 KB scanned with no leaks found. Dependency review detected no introduced High-or-higher vulnerable package and no denied package. P4-11 introduced no dependency or schema migration. Implementation PR #165 was merged into `main` with merge commit `d92408bf6ab24f30abeb3d1fb138f0fe891ab43c`. No P4-12 Golden Retrieval Dataset, Recall@K/Precision@K/MRR quality thresholds, P4-13 release isolation suite, production retrieval orchestration, query expansion, new reranking policy, ContextAssembler, `SOURCE_CONTEXT`, prompt/AI generation, frontend inspector UI, admin-role system, source preview/delivery, signed URLs, external dependency, migration, or other later-phase behavior was introduced. All P4-11 implementation, Expected Result, Definition of Done, validation, external review, independent security review, exact-head CI, and merge gates are satisfied.
- **Notes / blockers:** *None*
...
- **Evidence / link:** P4-12 implemented Golden Retrieval Dataset v1 in implementation PR #167 using versioned resources under `backend/src/test/resources/rag/golden-retrieval/v1/`: `dataset.json`, `corpus.json`, `baseline.json`, `thresholds.json`, and `README.md`. Dataset v1.0.0 contains 10 deterministic Anatomy/Physiology retrieval cases spanning EXACT, SEMANTIC, and MIXED query categories, with authorized synthetic fixtures, expected chunks/sections, acceptable alternatives, explicit irrelevant negatives, K values `[1,3,5]`, and primary K=3. Evaluation exercises production PostgreSQL/pgvector retrieval components through case-specific `BuildRetrievalScope`, lexical search, deterministic direct query vectors through `VectorSearchRepository`, and existing `HybridCandidateMerger`; no live `EmbeddingPort`, Gemini/Ollama/OpenAI provider, or external AI service participates. Metrics are calculated separately for Lexical, Vector, and Hybrid channels using Recall@K and Precision@K at K=1/3/5, plain full-ranked-list MRR using only expected chunks, primary-K expected-section hit rate, and explicit irrelevant-context rate. Typed `GoldenRetrievalBenchmarkContract` validation enforces exact dataset version, primary K, K values, case/channel/K coverage, finite normalized metrics, plain-MRR semantics, observed-versus-committed baseline equality, and v1 baseline-as-threshold equality; `baseline.json` and `thresholds.json` are intentionally identical for v1 so thresholds cannot be silently relaxed, missing, or auto-lowered. Resource validation rejects blank queries, invalid/non-finite/zero query vectors, invalid vector dimensions, broken MaterialVersion→Material or DocumentNode→MaterialVersion relationships, invalid extraction methods, duplicate/non-positive chunk indexes, non-finite embeddings, unauthorized expected/acceptable/irrelevant evidence, and insufficient authorized candidates for maximum K. The seeder persists declared chunk indices directly, keeping evaluation order deterministic. Visual relevance is explicitly **NOT MEASURED** in v1 because no production visual-retrieval search path exists in P4-12 scope; no visual metric or later retrieval behavior was fabricated. Focused Golden Retrieval, lexical, vector, hybrid, RetrievalScope, benchmark-contract, MRR-semantics, and architecture validation passed **91 tests, 0 failures, 0 errors, and 0 skips**. Required backend validation via `node scripts/validation/validate.mjs backend` returned **VALIDATION PASS** with Maven verification completing **962 tests, 0 failures, 0 errors, and 9 skips**; packaged backend build and `git diff --check` passed. External implementation review at final implementation head `dfc8eba6ef5e60c402ff715f0eafb6f162a5dda7` returned **APPROVED** with no unresolved benchmark-integrity, correctness, architecture, authorization, validation, or P4-12 scope blocker. The mandatory independent `hippocampus-security-vulnerability-review` at the same exact head returned **SECURITY PASS**, with no unresolved Critical, High, or Medium finding; review verified test-only production impact, synthetic fixture privacy, authorization through production `BuildRetrievalScope`, malformed benchmark fail-closed behavior, parameterized PostgreSQL fixture access, absence of secrets/live provider calls, and no runtime authorization bypass introduced by the benchmark. Exact-head GitHub Actions quality workflow #470 (`35447620687`) completed **SUCCESS** with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, `phase2-gate`, and `phase3-gate` successful. The security job scanned the exact implementation range `aa7d121b5489edb47eac835c0761193e336c5554..dfc8eba6ef5e60c402ff715f0eafb6f162a5dda7` using pinned Gitleaks v8.28.0: 4 commits / approximately 163.89 KB scanned with no leaks found; dependency review found no introduced High-or-higher vulnerable package and no denied package. P4-12 introduced no production retrieval tuning, new ranking/fusion algorithm, migration, runtime evaluation table/service, API endpoint, provider dependency, frontend behavior, P4-13 isolation-suite implementation, or P4-14 Phase 4 gate behavior. Implementation PR #167 was merged into `main` with merge commit `8091bcbee078c408352988917709c4402cb9a7aa`. The Golden Retrieval Dataset, deterministic baseline, recorded/enforced thresholds, measurable pre-AI retrieval quality baseline, required validation, external review, independent security gate, exact-head CI, merge, and Definition of Done are satisfied.
- **Notes / blockers:** *None*
...
- **Authority:** Documents 22,25
- **Evidence / link:** P4-13 implemented the dedicated PostgreSQL/Testcontainers `RagIsolationReleaseIntegrationTests` release suite in implementation PR #170, exercising the production `BuildRetrievalScope`, `JdbcRetrievalScopeSourceRepository`, `JdbcLexicalSearchRepository`, `JdbcVectorSearchRepository`, and `HybridCandidateMerger` with wholly synthetic deterministic fixtures. The six release scenarios cover semantically identical User A/User B corpora; a deliberately stronger foreign perfect-vector candidate; lexical, vector, and hybrid isolation; forged foreign-only and mixed authorized/foreign MaterialVersion scopes; foreign Topic access through `BuildRetrievalScope`; inactive Topic and inactive MaterialTopicLink exclusion; deleted Material exclusion; historical/non-current MaterialVersion exclusion; inactive Chunk exclusion; node-restricted retrieval against a stronger sibling-node candidate; forged foreign-user DocumentNode scope; and empty-scope fail-closed behavior. Assertions explicitly verify authorized chunk/material/version/node identities and known forbidden IDs rather than relying only on result counts. The production authorization path remains server-owned: `BuildRetrievalScope` derives identity from `CurrentUser`, scope-source persistence requires active same-user Subject/Topic/link/Material/current-version relationships, and the lexical/vector repositories independently retain ownership, non-DELETED Material, active-current-version, active-Chunk, and authorized version/node predicates for defense in depth against manually forged scopes. Review confirmed those predicates are applied before lexical ordering/limit and before vector similarity ranking/limit, so foreign relevance cannot override authorization. User validation passed all 6 P4-13 tests, 13 lexical repository integration tests, 11 vector repository integration tests, 2 retrieval-scope repository integration tests, and all 11 architecture tests. `node scripts/validation/validate.mjs backend` returned **VERDICT PASS**; `mvnw.cmd -B -ntp clean verify` completed with **968 tests, 0 failures, 0 errors, and 9 skips**, the packaged backend build succeeded, and repository-root `git diff --check` passed. External implementation review at exact head `d80702528b994465dccd92e53b84eac54fbcbed0` returned **APPROVED** with no unresolved correctness, architecture, authorization, lifecycle-isolation, or P4-13 scope blocker. The mandatory independent `hippocampus-security-vulnerability-review` at the same exact head returned **SECURITY PASS**, with no unresolved Critical, High, or Medium finding; the review verified server-owned authorization, cross-user/BOLA resistance, authorization-before-ranking, forged-scope defense in depth, deleted/historical/inactive evidence exclusion, node-scope isolation, parameterized fixture SQL, fail-closed empty/foreign scopes, and absence of live AI/provider involvement. Exact-head GitHub Actions quality workflow #476 (`35451756411`) completed **SUCCESS** with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, `phase2-gate`, and `phase3-gate` successful. The security job scanned exact PR range `3f168f9308b9efa678371828568dbcd840e24923..d80702528b994465dccd92e53b84eac54fbcbed0` using pinned Gitleaks v8.28.0: 2 commits / approximately 27.56 KB scanned with no leaks found; dependency review found no introduced High-or-higher vulnerable package and no denied package. P4-13 introduced no production retrieval change, migration, dependency, ranking/fusion modification, query expansion, reranking, provider behavior, API/controller, frontend behavior, or P4-14 implementation. Implementation PR #170 was merged into `main` with merge commit `96c2aa50ec1a3722b8d80a7a598a6f301dbc096f`. The expected zero-unauthorized-chunk result, automated release gate, required validation, external review, independent security gate, exact-head CI, merge, and Definition of Done are satisfied; P4-14 remains a separate Not Started Phase 4 gate task.
- **Notes / blockers:** *None*
...
- **Definition of Done:** Gate evidence recorded.
- **Authority:** Documents 26
- **Evidence / link:** P4-14 implemented the final Phase 4 retrieval gate in implementation PR #172 at exact reviewed head `895feeec6126cfc360e1b6de75b6c07f2a0134b0`, adding `Phase4RetrievalGateIntegrationTests` and the durable GitHub Actions `phase4-gate` while making no production `src/main` change. The gate loads and seeds the existing deterministic Golden Retrieval Dataset v1 corpus and composes existing production Phase 4 boundaries through an authenticated synthetic owner: `BuildRetrievalScope`, `LexicalSearchRepository`, `VectorSearchRepository`, the active index generation, deterministic Golden query vectors, `HybridCandidateMerger`, `BuildEvidencePackage`, `MaterializeEvidenceSourceReferences`, and authenticated `ResolveSourceReference`. The representative Anatomy exact scenario uses Golden case `case-anatomy-exact-roots` / query `C5-T1 brachial plexus roots`; it verifies that only the declared Anatomy MaterialVersion is authorized, executes production lexical and pgvector retrieval, requires the expected Golden chunks within `primaryK=3`, verifies an expected-section hit and committed explicit-irrelevant-context behavior, builds a `RetrievalQuality.STRONG` EvidencePackage, proves every selected chunk remains allowed by the RetrievalScope with correct Material/MaterialVersion provenance, verifies package diagnostics against the selected hybrid candidates, materializes the package's canonical chunk SourceReferences, and resolves each reference through the authenticated materials-owned source-resolution path. The representative Physiology semantic scenario uses `case-physio-semantic-sa-node` / query `What is the heart's primary pacemaker?` and executes the same integrated path, requiring the expected SA-node evidence within primary K and verifying authorized Physiology evidence/provenance through canonical source resolution. The visual-context scenario uses `case-anatomy-mixed-radial-nerve` / query `radial nerve wrist extension`; after normal text retrieval remains the entry point, the fixture creates one deterministic synthetic `SUPPORTED` `ANATOMY_DIAGRAM` with caption `Synthetic radial nerve diagram` under the same authorized radial-nerve MaterialVersion/DocumentNode and links it to retrieved `chunk-radial-1` using the existing `NEARBY` relationship. `BuildEvidencePackage` must then include that linked visual, and the gate verifies its visual ID, Material, MaterialVersion, DocumentNode, type, interpretation status, linked chunk, retrieval diagnostics, VISUAL `EvidenceSourceReference`, canonical persisted SourceReference, and authenticated resolution alongside the corresponding CHUNK references. This intentionally validates linked visual context rather than inventing visual search or visual embeddings; Golden Retrieval v1's existing limitation that visual retrieval relevance is **NOT MEASURED** remains unchanged. P4-14 does not duplicate the complete Golden benchmark: `GoldenRetrievalEvaluationIntegrationTests` remains the authoritative all-case baseline/threshold suite, and no change was made to `dataset.json`, `corpus.json`, `baseline.json`, or `thresholds.json`. It likewise does not duplicate the P4-13 adversarial isolation suite; P4-13 remains a required validation input while P4-14 independently verifies that each representative scope contains only the expected authorized MaterialVersions. Required user validation passed `Phase4RetrievalGateIntegrationTests`, `GoldenRetrievalEvaluationIntegrationTests`, `SourceReferenceIntegrationTests`, `RagIsolationReleaseIntegrationTests`, and `HippocampusArchitectureTests`; `node scripts/validation/validate.mjs backend` returned **VERDICT PASS**, including broad backend `clean verify` and `git-diff-check`. External implementation review at exact head `895feeec6126cfc360e1b6de75b6c07f2a0134b0` returned **APPROVED**, with no unresolved correctness, architecture, EvidencePackage, source-provenance, authorization, Golden-baseline, phase-boundary, or CI-gate blocker. Exact-head GitHub Actions quality workflow #480 (`35455390804`) completed **SUCCESS** with `backend-quality`, `frontend-quality`, `auth-e2e`, `security`, `phase1-gate`, `phase2-gate`, `phase3-gate`, and `phase4-gate` successful. The new `phase4-gate` uses `if: always()`, fails unless every prior quality/phase prerequisite is successful, checks out the exact implementation, uses the approved Temurin Java 25.0.4 runtime, and runs the isolated `Phase4RetrievalGateIntegrationTests` without unnecessarily duplicating full backend verification or installing the Phase 3 OCR runtime. The exact-head security job used pinned Gitleaks v8.28.0 against range `a7b9dcf0cea5b0c9c2bfaad426faa5b472dce87c..895feeec6126cfc360e1b6de75b6c07f2a0134b0`, scanning 1 commit / approximately 18.38 KB with no leaked secret found; dependency review found no introduced High-or-higher vulnerable package and no denied package. Implementation PR #172 was merged into `main` on 2026-09-19 as merge commit `56c8f7a75317b5ad84996b29af42d1650dc2ead6`. The mandatory independent `hippocampus-security-vulnerability-review` was then completed post-merge against the exact unchanged implementation head because the PR had been merged before that separate gate was run; its verdict is **SECURITY PASS**, with no unresolved Critical, High, or Medium finding and no manual-security-review requirement. The scoped adversarial review verified that the change adds only integration-test, CI, and tracker surface; exercises authenticated server-owned retrieval and source-reference authorization rather than bypassing it; keeps source material and synthetic visual content as data; uses parameterized fixture SQL; introduces no secret, dependency, migration, API, frontend, provider, prompt, AI-call, or production retrieval path; preserves P4-13's authorization-before-ranking and zero-leakage protections; and adds no Phase 5 behavior. P4-14 therefore introduces no production retrieval orchestration, lexical/vector ranking modification, hybrid-fusion tuning, reranking, query expansion, visual search, visual embeddings, AI provider behavior, prompt infrastructure, database schema, dependency, controller/API, frontend behavior, or Learning Engine behavior. The Expected Result and Definition of Done are satisfied: representative Anatomy, Physiology, and linked-visual contexts successfully traverse the completed authorized Phase 4 retrieval/evidence/provenance pipeline without generative AI; required validation, external review, independent security gate, exact-head CI, merge, and phase-level evidence are complete; Phase 4 Gate State is `PASS`; and Phase 5 may begin.
- **Notes / blockers:** *None*
...
- **Evidence / link:** Provider-independent canonical prompt-context construction implemented under `backend/src/main/java/com/hippocampus/ai/application/prompt/` in implementation PR [#179](https://github.com/KennethVier/hippocampus/pull/179). The implementation introduced immutable `PromptContext` retained-source provenance, caller-supplied `PromptTokenBudget`, the provider-neutral `PromptTokenCounter` seam, and `PromptContextBuilder` rendering canonical P5-01 `AiTaskRequest` values through the centralized P5-02 `PromptTemplateRegistry`. Context construction consumes only the already-authorized `EvidencePackage`; it does not perform retrieval or broaden RAG scope. The builder provides deterministic compact learner/activity serialization, ranked whole-source selection and exact-content deduplication, reserved output capacity, fail-closed mandatory-context handling, explicit `SOURCE_CONTEXT` and `STUDENT_RESPONSE` boundaries, delimiter escaping, and single-pass template substitution so source/student placeholder-like content cannot be promoted into trusted instructions. Structured-output repair schema ownership was corrected during external review: repair schemas are now derived by `PromptTemplateRegistry` from the canonical registered primary output contracts instead of being duplicated in `PromptContextBuilder`, preserving the active V1 prompt definitions and their SHA-256 regression snapshots. Focused tests cover all six canonical task types, deterministic prompt construction, token budgeting, learner/history reduction, source ranking/deduplication/provenance, mandatory-input overflow, minimal context, source/student/repair prompt-injection fixtures, and canonical repair-schema derivation. Required implementation validation was recorded before review, and the corrected exact implementation head `654f4c3091ba65c71fdef79428b8cd20beb940d1` passed GitHub Actions `quality` run #498 (ID `35605000500`), including `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, `phase2-gate`, `phase3-gate`, and `phase4-gate`. External implementation review returned `APPROVED` after the repair-schema ownership finding was remediated. Independent security vulnerability review returned `SECURITY PASS` for the final reviewed head with 0 Critical, 0 High, 0 Medium, and 0 Low findings and no manual security review required. PR #179 (`P5-03: Implement PromptContextBuilder`) was merged into `main` on 2026-09-21. Merge commit: `da60705bc7779f0060c2095112a2cdbd1eaf7296`. Expected behavior and Definition of Done are satisfied: prompt context is bounded, deterministic, source-traceable, preserves trusted instruction versus untrusted source/student-data boundaries, reserves output capacity, and remains provider-independent. Provider routing, Gemini/Ollama adapters, provider execution, output validation, source-reference validation, repair execution, fallback, and later Phase 5 responsibilities remain outside P5-03.
- **Notes / blockers:** _None_
...
- **Evidence / link:** Centralized deterministic provider routing implemented under `backend/src/main/java/com/hippocampus/ai/application/routing/` in implementation PR [#181](https://github.com/KennethVier/hippocampus/pull/181). The implementation introduced the application-owned `GEMINI` and `OLLAMA_CLOUD` provider identities, immutable `ProviderRoutingCandidate` and `ProviderRoute` contracts, configurable `COST_THEN_LATENCY` and `LATENCY_THEN_COST` routing policies, deterministic routing priority and provider/model tie-breaking, and fail-closed `ProviderRouteUnavailableException` handling. Route eligibility requires canonical task support, evaluation approval, current availability, available quota, and available rate-limit capacity. Provider selection contains no hard-coded Gemini/Ollama preference: application-supplied configuration may select either provider as primary. The highest-ranked eligible candidate becomes primary, while fallback is restricted to the highest-ranked eligible candidate from a different provider; alternate models from the same provider cannot masquerade as cross-provider fallback. Multiple configured models per provider remain supported. Candidate task sets are defensively copied, evaluation-approved tasks must be a subset of technically supported tasks, invalid model identifiers and negative routing ranks are rejected, and routing remains independent of input candidate ordering. `ProviderRouterTests` provide focused route-matrix coverage across all current canonical `AiTaskType` values, including `STRUCTURED_OUTPUT_REPAIR`, task/evaluation eligibility, availability, quota/rate-state exclusion, both configurable ranking strategies, provider-preference inversion, deterministic tie-breaking, cross-provider fallback, no-fallback behavior, fail-closed routing, defensive copying, and invalid configuration rejection. Agent-run focused validation passed with 14 `ProviderRouterTests`. Required user validation passed for `ProviderRouterTests`, `AiTaskContractsTests`, `HippocampusArchitectureTests`, broad backend validation, and `git diff --check`. The final reviewed implementation head `7a714d23d9cba0c9da51bff936dbb3586cb9060b` passed exact-head GitHub Actions `quality` run #502 (ID `35620113313`), including `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, `phase1-gate`, `phase2-gate`, `phase3-gate`, and `phase4-gate`. External implementation review returned `APPROVED`. Independent security vulnerability review returned `SECURITY PASS` with 0 Critical, 0 High, 0 Medium, and 0 Low findings and no manual security review required. The review confirmed that P5-04 introduces no provider HTTP execution, credential handling, externally reachable API surface, persistence, database access, new dependency, or additional authorization/RAG surface, and that unsupported, unevaluated, unavailable, quota-exhausted, or rate-limited candidates fail closed rather than being routed. PR #181 (`P5-04: Implement ProviderRouter`) was merged into `main` on 2026-09-21. Merge commit: `6de6da951b08e532ecd4a302316bad01c1f35b82`. Expected behavior and Definition of Done are satisfied: canonical AI tasks are routed centrally and deterministically without Learning/domain code selecting Gemini or Ollama. Gemini/Ollama provider adapters, provider execution, request management, retries/backoff, timeout/cancellation, output validation, source-reference validation, repair execution, fallback execution, usage diagnostics, and later Phase 5 responsibilities remain outside P5-04.
- **Notes / blockers:** _None_
...
- **Evidence / link:** Implemented the provider-neutral execution and streaming contracts and Spring AI Google GenAI adapter under `backend/src/main/java/com/hippocampus/ai/`, including route-selected model overrides, canonical system/task prompt mapping, JSON response mode, reserved output-token mapping, raw untrusted text-delta and terminal stream events, provider-reported usage, latency, typed safe failures, and disabled-by-default server-side credential configuration. PR #183 correction adds Spring AI streaming behind a provider-neutral callback contract (no Reactor or provider DTO leakage), explicitly configures one provider attempt with no application-level retry, classifies Google GenAI SDK `ApiException` status codes safely from the cause chain, and removes the divergent properties-level model default so model selection remains configuration/evaluation driven. Correction-focused agent validation passed: 4 `AiProviderAdapterContractTests`, 3 changed `GeminiProviderAdapterTests`, and 2 changed `GeminiProviderConfigurationTests`, all with 0 failures and 0 errors. The opt-in live smoke class now covers the streaming path but was not agent-run. Previous broad/user validation and the previous successful non-streaming live smoke predate these corrections; user revalidation and rerunning the updated Gemini live smoke remain required before review completion.
- **Notes / blockers:** _None_
...
- **Evidence / link:** Implemented the provider-neutral execution and streaming contracts and remote Ollama Cloud REST adapter under `backend/src/main/java/com/hippocampus/ai/`, including the HTTPS `/api/chat` integration, infrastructure-only bearer authentication, route-selected model and canonical system/task prompt mapping, structured JSON mode, raw untrusted output, provider-reported usage, latency, safe HTTP/network/response failure normalization, cloud-only URL validation, and disabled-by-default server-side credential configuration. PR #183 correction adds real Ollama NDJSON streaming with `stream=true`, incrementally normalizes provider text deltas and terminal model/usage/finish metadata, and keeps provider DTOs and transport details inside infrastructure while exposing no Reactor type. Correction-focused agent validation passed: 4 `AiProviderAdapterContractTests` and 2 changed Ollama streaming adapter tests, all with 0 failures and 0 errors. The opt-in live smoke class now covers the streaming path but was not agent-run. Previous broad/user validation and the previous successful non-streaming live smoke predate these corrections; user revalidation and rerunning the updated Ollama live smoke remain required.
- **Notes / blockers:** _None_
