...
- **Authority:** Documents 21
- **Evidence / link:** Review candidate implements deterministic P3-08 structure detection from durable PAGE_TEXT evidence plus a provider-neutral, PDFBox-backed supplementary inspection boundary. It handles bounded bookmark destinations, TOC corroboration, conservative numeric font-weight/ForceBold evidence, relative font/layout/spacing, numbering, and repeated positional regions; emits only CHAPTER/SECTION/SUBSECTION nodes with NATIVE or HEURISTIC provenance and HIGH/MEDIUM/LOW confidence; preserves the DOCUMENT root and PAGE_TEXT rows; and performs exact semantic replay or fail-closed conflict detection with atomic JDBC insertion in a separate short `@Transactional` application bean. PDF download, MIME inspection, PDFBox parsing, page-local layout analysis, OCR uncertainty handling, and deterministic hierarchy calculation remain outside the write transaction. Native inspection reuses `PdfExtractionProperties.pageBatchSize()`, `maxPages()`, and `maxNativeTextCharsPerPage()`, retains raw positions/lines for one page/batch only, and enforces outline count/depth/title/destination/cycle, text-position, aggregate retained native text, line, candidate, and node ceilings; a synthetic 601-page fixture completed in batches no larger than 32. Correction following external review deduplicates repeated header/footer signatures once per physical page, makes exact native-outline/body confirmation confidence OCR-quality aware, and locks the Material lifecycle row along with the target MaterialVersion and DOCUMENT root before detected-structure writes. Focused correction validation passed: detector/application/pipeline 19/19, PDF structure inspector/configuration 8/8, PostgreSQL/Testcontainers detected-structure persistence 4/4, and architecture 9/9. External general implementation review for reviewed head `0e340cc2c2656a17084ae307723cd14b8cc67c4c` was **REQUEST CHANGES**. Quality run #298 / `34047105391` on exact head `0e340cc2c2656a17084ae307723cd14b8cc67c4c`: `frontend-quality` success, `security` success, `auth-e2e` success, `backend-quality` failed during Spotless verification, and phase gates failed downstream. No external implementation approval, independent security approval, merge, or `Done` status is claimed. No new dependency, migration, ADR, provider/AI integration, rendering/OCR invocation, later parsing task, frontend, or architecture deviation was introduced.
- **Notes / blockers:** Local `clean verify` on the correction candidate compiled all 246 main and 93 test sources, ran 424 tests, and failed with 8 failures, 131 errors, and 4 skips because of existing Windows-incompatible test assumptions, including `/usr/bin/tesseract` being rejected as a non-absolute Windows path across application-context integration tests, PDFBox extraction assertions expecting LF while Windows PDFBox output used CRLF, and local Tesseract CLI test assumptions reporting `ENGINE_UNAVAILABLE` for expected process-output cases. The requested local `spotless:apply` and `spotless:check` commands could not run because this checkout has no configured Maven Spotless plugin; Maven returned `No plugin found for prefix 'spotless'`. Authoritative GitHub Linux CI/full-suite validation remains required before `Done`; this is an explicit environment limitation, not a claimed pass. P3-08 is not externally approved, merged, or complete.
