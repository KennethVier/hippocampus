...
- **Evidence / link:** P3-14 implementation adds V13 Chunk persistence, deterministic CHUNKER_V1 identity, bounded hierarchy-aware PAGE_TEXT/TABLE_TEXT chunking, exact ordered TextBlock links, conservative visual links, transactional replay verification, typed configuration, and CHUNK stage registration. Final correction freezes the accepted CHUNKER_V1 namespace with a golden UUID literal and adds PostgreSQL integration suites for chunk persistence, exact replay/created-at preservation, compatible convergence, full source snapshots, ordered source links, exact-primary-page visual links, independent finalization, configurable hard limits, and transactional rollback. Correction-focused non-container validation passed 50 tests with zero failures/errors/skips. The complete 101-test focused command was attempted, but 51 Docker-backed cases could not initialize because the Docker socket is unavailable; authoritative exact-head CI remains required before fresh general review.
- **Notes / blockers:** _None_
