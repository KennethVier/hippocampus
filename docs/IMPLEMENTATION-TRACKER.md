...
- **Authority:** Documents 21
- **Evidence / link:** P3-11 implementation extends the existing `VISUAL_EXTRACT` handler so persisted P3-10 visuals are followed by visual-context association before the unchanged `NORMALIZE` transition. A deterministic domain policy associates context only when one visual and one explicit numbered `Figure`/`Fig.` caption candidate occur on the same physical page, supports one bounded multiline continuation, and derives only a bounded immediately following explanatory region. Missing or ambiguous evidence remains unresolved without failing the stage. JDBC persistence validates same-version visual/node/page provenance, fills only NULL context, treats exact replay as a no-op, preserves conflicting/manual enrichment, and never creates assets or mutates extraction/classification fields. No migration, dependency, processing stage, or later-phase capability was added. Initial focused validation passed 48 tests with zero failures/errors/skips across association policy/application/handler/configuration, visual context and P3-10 persistence integration, processing sequencing, and architecture. External review of head `c3b2823a079e29a0592578b821ce0b4f568aa702` identified three correction findings: extraction quality was ignored, all visual-page `PAGE_TEXT` bodies accumulated inside one document-wide transaction, and every visual asset remained locked throughout that evaluation. The correction now permits native page text and only strong-quality OCR for confident caption/nearby-text association; limited/poor OCR remains unresolved. It loads lightweight visual metadata without document-wide locks, validates assets and same-version nodes once, groups metadata by page, loads and evaluates only one page's `PAGE_TEXT` at a time, discards that page text before continuing, and persists each detected page association through the short transactional `PersistVisualContext` boundary, which re-reads and locks only its target visual. Correction-focused validation passed 17 tests with zero failures/errors/skips across policy, incremental application flow, handler sequencing, bean registration, and JDBC persistence/locking behavior.
- **Notes / blockers:** _None_
