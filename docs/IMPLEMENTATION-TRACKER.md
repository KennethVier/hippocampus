...
- **Evidence / link:** P4-09 implements a provider/framework-independent immutable `EvidencePackage` contract containing ranked `EvidenceChunk` values, authorized/deduplicated `EvidenceVisual` values, package-local machine-provenance `EvidenceSourceReference` values, explicit `RetrievalQuality`, typed deterministic limitations, and factual immutable `RetrievalDiagnostics`. `BuildEvidencePackage.Command` accepts only the server-owned `RetrievalScope`, already-ranked `HybridCandidate` values, explicit retrieval quality, and caller-selected chunk/visual budget. It preserves candidate order and verbatim source content, applies bounded chunk/visual selection, derives selected vector index-generation IDs, rejects duplicate chunks, and revalidates every candidate against centralized whole-version/node scope. `JdbcEvidenceVisualRepository` uses bound parameters and fixed SQL fragments across `chunk_visual_links`, active `chunks`, `material_versions`, owning/non-deleted/current `materials`, and same-version `visual_assets`; empty selected sets skip SQL. Visual authorization is independently checked at visual-node level, so node-restricted scopes exclude other-node and null-node visuals while whole-version and mixed scopes retain their intended breadth. SUPPORTED visuals are included; LIMITED visuals are included with `LIMITED_VISUAL_INTERPRETATION`; UNASSESSED/UNSUPPORTED/FAILED visuals are excluded with a typed limitation. Multi-chunk links merge into one visual with unioned link/relationship sets and fail closed on provenance/metadata disagreement. Package integrity rejects dangling/duplicate/mismatched source references and inconsistent diagnostics; multi-page chunks do not fabricate a page target. P4-10 canonical SourceReference persistence, stable reference IDs, display labels, lookup/opening, and resolution remain explicitly deferred; no migration, controller, frontend, ranking, prompt, provider, dependency, or P4-10+ behavior was introduced. Focused validation passed 24 P4-09 domain/application/PostgreSQL tests, plus 3 centralized scope tests and all 10 architecture tests (37 total), including production-composed Testcontainers coverage for linked/unlinked visuals, metadata preservation, merged links, interpretation filtering, ownership/deleted/historical/inactive exclusion, node-only/whole-version/mixed authorization, deterministic visual budgeting/order, and empty sets. Required `node scripts/validation/validate.mjs backend` returned **VALIDATION PASS** with 908 tests, 0 failures, 0 errors, and 9 skips; `git diff --check` passed. General implementation review returned **APPROVED** with no unresolved correctness, architecture, scope, or validation finding. The independent scoped `hippocampus-security-vulnerability-review` returned **SECURITY PASS** with no blocking Critical, High, or Medium finding; it verified authorization-before-packaging, defense-in-depth ownership/current-version/active-source predicates, visual-level node isolation, selected-link provenance agreement, parameterized SQL, prompt-injection text preservation as inert data, fail-closed exceptional behavior, absence of source-text logging/private storage-key exposure, and no dependency or public attack-surface expansion.
- **Notes / blockers:** _None_
