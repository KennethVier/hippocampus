...
- **Notes / blockers:** P3-07 remains `Ready for Review`, not `Done`. Resource-safety review correction adds a PDFBox-allocation-matched preflight that small positive `UserUnit` values cannot reduce, painted source-image dimension/per-image/page pixel guards including masks, renderer subsampling, and success/failure page-resource cache cleanup. External implementation and independent security re-review remain required; no approval or security verdict is claimed here.
