...
- **Authority:** Documents 21
- **Evidence / link:** P4-04 implements the durable provider-neutral `EMBED` stage. `EmbedMaterialVersion` reads only active chunks for one non-deleted material version in deterministic `chunk_index` order, submits bounded batches of unchanged canonical content through `EmbeddingPort` with chunk UUID correlation, verifies provider cardinality/reference/model/dimension compatibility, checks processing-job ownership before and after each provider call, and reports durable embedded/eligible counts. Provider calls run outside database transactions; each successful batch is persisted by the short transactional `PersistEmbeddingBatch` collaborator through `JdbcEmbeddingJobRepository`. PostgreSQL persistence uses generation-scoped `ON CONFLICT (chunk_id,index_generation_id) DO NOTHING`, locks and verifies the owning material/version as non-deleted before writing, validates every chunk is active and belongs to the requested material version, preserves an existing valid row on replay, and retains the V15 pgvector dimension trigger. Generation selection prefers exactly one `ACTIVE`, otherwise exactly one `BUILDING`, fails closed on ambiguity or incompatibility, and concurrency-safe initial creation records actual first-response provider/model/version/dimension metadata with `CHUNKER_V1`. Processing progression is now handler-availability based: `CHUNK` can durably enqueue `EMBED` only when its handler is present, while `EMBED` stops before unavailable `INDEX`; normal startup remains valid without an enabled `EmbeddingPort`. Stable recovery classification maps provider failure to transient `EMBEDDING_PROVIDER_UNAVAILABLE` and invalid response to fatal `EMBEDDING_RESPONSE_INVALID`. Deterministic tests prove 7-at-3 batching (`3/3/1`), canonical input/correlation, durable progress, inactive and deleted-material exclusion, deletion fencing before work, between batches, and between provider return and persistence, provider calls outside transactions, partial-success durability and retry of only missing chunks, zero-call replay idempotency, malformed/mismatched response rejection, ownership-loss fencing, compatible concurrent initial generation creation, pgvector persistence/dimension enforcement, generation-scoped uniqueness, fail-closed ambiguous generation selection, mixed-material batch rejection with complete transaction rollback, and real `CHUNK -> EMBED` completion without speculative `INDEX`. Focused validation passed 19 tests (7 unit and 12 PostgreSQL/Testcontainers), with 0 failures, 0 errors, and 0 skips. Required `node scripts/validation/validate.mjs backend` returned **PASS**: clean Maven verification executed 809 tests with 0 failures, 0 errors, and 9 skips; `git diff --check` passed. Implementation-time review and security assessment found no additional blocking issue in the validated surface. External implementation review and the mandatory independent security vulnerability review remain completion gates. No schema migration, new dependency, credential exposure, provider-specific exception handling, retrieval/index/activation/reindex implementation, frontend work, or P4-05+ behavior was introduced. Final validation verdict: **VALIDATION PASS**.
- **Notes / blockers:** _None_
