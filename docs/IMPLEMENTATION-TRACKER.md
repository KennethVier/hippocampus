...
- **Authority:** Documents 18,22
- **Evidence / link:** P4-10 adds materials-owned canonical SourceReference persistence and authorization-safe resolution without implementing P4-11+ behavior. Flyway V18 creates `source_references` with the authoritative columns, restrictive composite provenance foreign keys, meaningful-target/page/timestamp/chunk-versus-visual checks, a canonical `UNIQUE NULLS NOT DISTINCT` identity, and target-resolution indexes. `SourceReference`, the sealed page/node/chunk/visual target contracts, `CreateSourceReferences`, `ResolveSourceReference`, `SourceReferenceRepository`, and `JdbcSourceReferenceRepository` remain owned by `materials`; an explicit architecture rule prevents materials from depending on `rag`. Both public use cases derive the owner exclusively from `CurrentUser.authenticatedUser().userId()`. Creation accepts only primary targets, rejects duplicate primary targets, resolves redundant provenance and learner-facing labels from authoritative PostgreSQL metadata, preserves input order, omits fabricated pages for multi-page chunks, and uses a concurrency-safe upsert that retains the canonical UUID while allowing the authoritative label to refresh. The transactional batch rolls back completely when any target is foreign, deleted, historical, inactive, mismatched, or otherwise invalid. Resolution returns only canonical IDs/page/timestamps/label/creation time and requires the same owning non-DELETED Material, current active MaterialVersion, same-version node/visual, active same-version Chunk, and valid known page bounds; foreign, missing, deleted, historical, inactive, and malformed references produce the same sanitized `SOURCE_REFERENCE_NOT_FOUND` application failure. `MaterializeEvidenceSourceReferences` maps P4-09 CHUNK/VISUAL package-local references through the materials creation contract in package order without mutating `EvidencePackage`; repeated materialization returns stable IDs and fails closed if authorization has changed. Focused P4-10 plus P4-09 regression and architecture validation passed 48 tests with 0 failures/errors, including 10 PostgreSQL/Testcontainers source-reference scenarios and all 11 architecture tests. The full-history Flyway test applies all 18 migrations from empty schema and is idempotent. Required `node scripts/validation/validate.mjs backend` returned **VALIDATION PASS** with 922 tests, 0 failures, 0 errors, and 9 skips; the packaged backend build and `git diff --check` passed. `git diff main...HEAD` is empty because these changes remain uncommitted. General implementation review returned **APPROVED**. The independent OWASP ASVS 5.0.0 / OWASP Top 10:2025 / API Security Top 10 review returned **SECURITY PASS** with no unresolved blocking Critical, High, or Medium finding, verifying BOLA/IDOR resistance, CurrentUser derivation, active/current/deleted fencing, parameterized SQL, uniform errors, provenance integrity, transaction rollback, absence of private-source exposure, and no dependency/secret change. No controller, signed URL, binary streaming, timestamp navigation behavior, retrieval inspector, golden dataset, full isolation suite, retrieval orchestration, ContextAssembler, prompt/provider behavior, frontend behavior, or other P4-11+ scope was introduced.
- **Notes / blockers:** _None_
