...
- **Definition of Done:** Tests pass.
- **Authority:** Documents 21
- **Evidence / link:** P3-12 is implemented in PR [#126](https://github.com/KennethVier/hippocampus/pull/126). The unchanged durable sequence remains `MATERIAL_VALIDATE -> MATERIAL_EXTRACT -> STRUCTURE_DETECT -> VISUAL_EXTRACT -> NORMALIZE -> CHUNK -> EMBED`; inside `VISUAL_EXTRACT`, table extraction follows P3-10 visual extraction and P3-11 visual-context association. Native extraction is page-bounded and conservative: reliable aligned layouts may contain two or more columns and rows, while borderless `STRONG` confidence requires at least three consistent rows, compact table-like cells, stable anchors, and no rotated, overlapping, or side-by-side ambiguity. `STRONG` output uses exact TAB/LF serialization; uncertain credible layouts remain `LIMITED` without invented TAB relations, and columnar prose produces no fabricated table. OCR accepts only repeated explicit TAB/pipe evidence, preserves quality without upgrade, scans lines and delimiters incrementally, and enforces candidate/table budgets before unbounded splitting or accumulation. Layout rows, cells, candidate rows, page/document tables, serialized characters, text positions, page text, and crop-box geometry are bounded during construction; malformed/off-page geometry cannot establish `STRONG` relations. TABLE_TEXT ordinals remain `pageCount + globalTableSourceOrder`, cross-page content is never stitched, and parsing/object-store work remains outside the short persistence transactions. `DocumentNodePageLocator` still selects the deepest containing same-version node with document-root fallback; JDBC persistence and finalization now independently require that the durable node's page interval contains each TABLE_TEXT page. Exact and partial replay remain idempotent and conflicts fail closed. The extractor page protocol validates both the last emitted page and emitted page-count claim against the returned count, including zero-table pages. PAGE_TEXT accepts only `NATIVE` with null quality or `OCR` with nonnull `STRONG`, `LIMITED`, or `POOR`; invalid combinations fail closed. The P3-04 PAGE_TEXT-only finalization fix remains intact. No migration, dependency, new processing stage/job type, table aggregate/schema, synthetic visual, AI/provider work, P3-13 work, or architectural deviation was introduced. Initial focused validation passed 44 tests. External general review of initial head `050891565ca65c8f46a2e295566e4e82b717c432` identified four findings: (1) a three-column confidence shortcut missed reliable two-column tables and insufficiently guarded columnar prose and side-by-side layouts; (2) several table resource and geometry budgets were applied only after large intermediate structures could be materialized; (3) durable node validation did not prove node/page containment; and (4) page-count protocol and invalid PAGE_TEXT quality combinations were not fully fail-closed. Corrective implementation commit `8b49a130ff53a6d227fbbae7a76debbc59fe2863` resolves all four findings. The exact requested focused Maven selection passed 52 tests with zero failures, errors, or skips, including `HippocampusArchitectureTests`, and `git diff --check` passed. P3-12 remains Ready For Review; external re-review, authoritative CI, independent security review, merge, and completion evidence remain required before Done.
- **Notes / blockers:** _None_
