...
- **Evidence / link:** P4-03 implements the initial direct Gemini embedding adapter as an uncommitted review candidate. The backend imports the official Spring AI BOM at `2.0.1` and adds only `org.springframework.ai:spring-ai-google-genai-embedding`; the resolved AI path is Spring AI `2.0.1`, Google GenAI `1.65.0`, and the required Spring AI model/commons/template/retry transitive modules, with no chat, vector-store, Ollama, agent, memory, or provider-router dependency. `GeminiEmbeddingConfiguration` is manually activated only by `hippocampus.rag.embedding.gemini.enabled=true`; normal startup remains credential-free while enabled configuration fails closed for missing/blank `GEMINI_API_KEY`, blank model, or non-positive dimension. Model candidate `gemini-embedding-2` and dimension `768` are typed configuration defaults and remain replaceable. No legacy embedding task type, retrieval prefix, normalization, truncation, splitting, or dimension-discovery call is configured. `GeminiEmbeddingAdapter` sends one unchanged text batch through Spring AI, correlates every result by its provider index back to the caller UUID, returns deterministic request order, validates exact cardinality/unique in-range indices/one result per input/exact configured dimension/finite immutable vectors, maps provider `google-genai`, uses valid response model metadata or the configured model fallback, leaves version absent, and maps Spring AI's aggregate embedding token count from total usage while preserving reported zero versus unavailable and rejecting negative usage. Provider/client and malformed-response failures cross the port only as fixed-message provider-neutral `EmbeddingFailureException` values without provider causes or sensitive payloads. Focused command `mvn -B -ntp "-Dtest=EmbeddingPortContractTests,GeminiEmbeddingAdapterTests,GeminiEmbeddingConfigurationTests,GeminiEmbeddingLiveSmokeTest,HippocampusArchitectureTests" test` passed 35 executed tests with 0 failures/errors and 1 explicitly gated live-smoke skip; this includes 11 unchanged P4-02 contract tests, 9 adapter tests, 5 configuration tests, and 10 architecture tests. The ArchUnit suite now proves `rag.port` has no Spring or Google dependency. Live command `HIPPOCAMPUS_GEMINI_LIVE_TEST=true GEMINI_API_KEY=<environment-injected> mvnw.cmd -B -ntp "-Dtest=GeminiEmbeddingLiveSmokeTest" test` was not authorized or executed because this environment reported neither variable available; the normal suite confirms the smoke is quota-safe and skipped, so live provider validation remains a factual external limitation rather than a claimed pass. Required `node scripts/validation/validate.mjs backend` finished with **PASS**: clean backend verification passed 787 tests with 0 failures/errors and 9 skips, and its `git-diff-check` passed (`.validation/summary.json`, 2026-09-14). One intervening clean run suffered missing unrelated compiler-generated `$1.class` files; a clean focused recompile then passed all 38 affected unrelated tests, classifying that run as a transient dependency/tooling failure, and the final clean broad run passed. Complete scope review found no P4-04 job/orchestration, persistence/upsert, repository, migration, `chunk_embeddings` write, `IndexGeneration` activation, retrieval, endpoint, frontend, prompt-policy, multimodal, Ollama, chat, or later-phase change. Architecture/dependency review found provider types contained in RAG infrastructure and provider-neutral port results. Secret/log review found no committed credential or full-input logging; credentials remain server-side and configuration/error strings are tested not to disclose them. Independent scoped security review verdict is **SECURITY PASS** with no unresolved blocking Critical, High, or Medium finding in the changed adapter/configuration/dependency surface. Required post-implementation verdict is **VALIDATION PAS** solely because the explicitly required real-provider smoke could not be authorized in this environment; deterministic adapter and broad backend validation passed, and the candidate is left at `Ready for Review` with that limitation documented. No ADR or undocumented architecture deviation was introduced.
- **Evidence follow-up (2026-09-14):** The user confirmed that `GEMINI_API_KEY` was set and `HIPPOCAMPUS_GEMINI_LIVE_TEST=true`, then ran `.\mvnw\.cmd -B -ntp "-Dtest=GeminiEmbeddingLiveSmokeTest" test` from `backend`. The real Gemini smoke completed with **BUILD SUCCESS**: 1 test executed, 0 failures, 0 errors, 0 skips, test time 1.467 seconds, total Maven time 4.234 seconds, finished at 2026-09-14T13:54:30+08:00. This production adapter path verified the real `EmbeddingPort` conversion, caller correlation, model metadata, and configured 768-dimensional finite vectors; deterministic tests separately verify reported, zero, unavailable, and negative usage handling. Combined with the passing focused, architecture, dependency, broad-backend, diff-hygiene, scope, and independent security evidence above, the authoritative post-implementation verdict is **VALIDATION PASS**. This follow-up supersedes the earlier skipped-smoke limitation and truncated verdict text; P4-03 remains `Ready for Review` pending external review and is not marked `Done`.
- **Notes / blockers:** _None_
