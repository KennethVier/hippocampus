...
- **Evidence / link:** Implementation PR [#77](https://github.com/KennethVier/hippocampus/pull/77) implements authenticated, CSRF-protected `POST /api/materials` with exactly one `file` multipart part and `201 Created`, storing the original through the existing streaming `BinaryObjectStore` before atomically persisting one owner-scoped Material plus MaterialVersion v1. Ownership derives from `CurrentUser`; client identity/path/storage-key input is not authoritative. Declared `application/pdf`, `image/jpeg`, `image/png`, and `text/plain` are accepted only as provisional transport MIME classification pending P2-11 content detection. The storage key uses a server-generated opaque UUID and excludes filename/client path; existing Hibernate-generated Material/MaterialVersion IDs remain unchanged. `Material.storage_key` remains null, `MaterialVersion.storage_key` holds the original key, both lifecycle states begin `UPLOADED`, and processing progress, `active_version_id`, and `activated_at` remain null; no ProcessingJob is introduced. Storage failure creates no relational rows; relational failure triggers idempotent compensating object deletion, with cleanup failure remaining a safe failure and documented orphan-object residual risk. PostgreSQL/Testcontainers coverage proves successful persistence, real filesystem round trip, same-filename cross-user owner/key isolation, and transaction rollback when version persistence fails. Typed application file-size limits are validated against Spring Boot multipart transport limits, and framework-level null-handler resolution proves pre-controller `MaxUploadSizeExceededException` returns sanitized `413 UPLOAD_TOO_LARGE`. Upload composition requires `CurrentUser`, `BinaryObjectStore`, and `MaterialUploadPersistence`; pilot remains fail-closed with no filesystem/upload route. External general implementation review at runtime head `ba9f9f5a747afb0ee2d513b610f05d304628e933` is **APPROVED** under `hippocampus-review-implementation`. Independent adversarial review under `hippocampus-security-vulnerability-review` is **SECURITY PASS** for the P2-06 changed surface: no unresolved blocking Critical, High, or Medium finding was identified in authentication/CSRF, owner authority, key/path control, preliminary MIME trust, resource bounds, safe exceptions, storage/database compensation, transactional rollback, or pilot composition. Quality run #158 (ID `33400021044`) succeeded on that reviewed runtime head with `backend-quality`, `frontend-quality`, `security`, `auth-e2e`, and `phase1-gate` all successful; the later tracker-only head requires its own replacement CI before merge. No migration, dependency, frontend, Tika/PDFBox, R2/Firebase, P2-07, P2-11, or P3-01 implementation was introduced. Remaining deferred limitations are explicit: MIME/content hardening and final intake policy remain P2-11; upload quota/concurrency/rate-limit policy is deferred; compensation-delete failure can leave an orphan binary; production/PILOT R2 remains deferred. P2-06 is Ready for Review, not Done.
- **Notes / blockers:** _None. Deferred P2-11 intake hardening, broader upload abuse controls, and production R2 do not block the P2-06 intake foundation._
