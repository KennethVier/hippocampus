...
test('student completes Subject and Topic organization CRUD', async ({ page }) => {
  ...
  const subjectCard = page.getByRole('heading', { name: subjectName }).locator('..').locator('..')
  ...
  await page.getByRole('button', { name: 'Save Subject' }).click()
  await expect(page.getByRole('heading', { name: editedSubject })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: editedTopic })).toHaveCount(0)
  ...
  await expect(page.getByRole('heading', { name: editedSubject })).toHaveCount(0)
})
