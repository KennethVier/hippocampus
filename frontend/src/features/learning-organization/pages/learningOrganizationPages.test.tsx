...
describe('Subjects page', () => {
  ...
  it('preserves hidden sortOrder and preserves non-empty description whitespace during edit', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Name'), { target: { value: 'Anatomy' } })
    fireEvent.change(screen.getByLabelText('Description'), { target: { value: '  meaningful description  ' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save Subject' }))
    await waitFor(() => expect(update).toHaveBeenCalledWith(subject.id, expect.objectContaining({ sortOrder: 9, description: '  meaningful description  ' })))
  })
  ...
  it('does not leak Create Subject errors into Edit Subject', async () => {
    vi.spyOn(api, 'listSubjects').mockResolvedValue({ items: [subject], page: 0, size: 12, totalElements: 1, totalPages: 1 })
    vi.spyOn(api, 'createSubject').mockRejectedValue(new ApiError({ kind: 'http', status: 409, code: 'SUBJECT_NAME_CONFLICT', message: 'hidden' }))
    renderAt('/subjects?page=1')
    fireEvent.click(await screen.findByRole('button', { name: 'Create Subject' }))
    fireEvent.change(screen.getByLabelText('Name'), { target: { value: 'Anatomy' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save Subject' }))
  ...
  it('does not leak Edit Subject errors into Create Subject', async () => {
    vi.spyOn(api, 'listSubjects').mockResolvedValue({ items: [subject], page: 0, size: 12, totalElements: 1, totalPages: 1 })
    vi.spyOn(api, 'updateSubject').mockRejectedValue(new ApiError({ kind: 'http', status: 409, code: 'SUBJECT_NAME_CONFLICT', message: 'hidden' }))
    renderAt('/subjects?page=1')
    fireEvent.click(await screen.findByRole('button', { name: 'Edit Subject' }))
    fireEvent.change(screen.getByLabelText('Name'), { target: { value: 'Anatomy' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save Subject' }))
...
describe('Subject detail', () => {
  ...
  it('renders active Topics as subordinate headings without linking to a Topic workspace', async () => {
    ...
    renderAt(`/subjects/${subject.id}`, true)
    expect(await screen.findByRole('heading', { level: 3, name: 'Thorax' })).toBeInTheDocument()
    expect(screen.queryByRole('link', { name: /Thorax|Open Topic/ })).not.toBeInTheDocument()
  })

  it('archives a Topic only after confirmation', async () => {
    vi.spyOn(api, 'getSubject').mockResolvedValue(subject)
    vi.spyOn(api, 'listTopics').mockResolvedValue({ items: [topic], page: 0, size: 12, totalElements: 1, totalPages: 1 })
    const archive = vi.spyOn(api, 'archiveTopic').mockResolvedValue({ ...topic, status: 'ARCHIVED' })
    renderAt(`/subjects/${subject.id}`, true)
    fireEvent.click(await screen.findByRole('button', { name: 'Archive Topic' }))
  ...
  it('restores authoritative Subject values after cancelling an edit draft', async () => {
    vi.spyOn(api, 'getSubject').mockResolvedValue(subject)
    vi.spyOn(api, 'listTopics').mockResolvedValue({ items: [], page: 0, size: 12, totalElements: 0, totalPages: 0 })
    renderAt(`/subjects/${subject.id}`, true)
    fireEvent.click(await screen.findByRole('button', { name: 'Edit Subject' }))
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Cancel' }))
    fireEvent.click(screen.getByRole('button', { name: 'Edit Subject' }))
    expect(screen.getByLabelText('Name')).toHaveValue(subject.name)
    expect(screen.getByLabelText('Description')).toHaveValue(subject.description)
  })

  it('maps Subject name conflicts during detail edits', async () => {
    vi.spyOn(api, 'getSubject').mockResolvedValue(subject)
    vi.spyOn(api, 'listTopics').mockResolvedValue({ items: [], page: 0, size: 12, totalElements: 0, totalPages: 0 })
    vi.spyOn(api, 'updateSubject').mockRejectedValue(new ApiError({ kind: 'http', status: 409, code: 'SUBJECT_NAME_CONFLICT', message: 'hidden' }))
    renderAt(`/subjects/${subject.id}`, true)
    fireEvent.click(await screen.findByRole('button', { name: 'Edit Subject' }))
    fireEvent.change(screen.getByLabelText('Name'), { target: { value: 'Existing Subject' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save Subject' }))
    expect(await screen.findByRole('alert')).toHaveTextContent('already have a Subject')
  })

  it('does not leak Create Topic errors into Edit Topic', async () => {
    vi.spyOn(api, 'getSubject').mockResolvedValue(subject)
    vi.spyOn(api, 'listTopics').mockResolvedValue({ items: [topic], page: 0, size: 12, totalElements: 1, totalPages: 1 })
    vi.spyOn(api, 'createTopic').mockRejectedValue(new ApiError({ kind: 'http', status: 400, code: 'VALIDATION_FAILED', message: 'hidden' }))
    renderAt(`/subjects/${subject.id}`, true)
    fireEvent.click(await screen.findByRole('button', { name: 'Create Topic' }))
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('Review the form')
    fireEvent.click(screen.getByRole('button', { name: 'Cancel' }))
    fireEvent.click(screen.getByRole('button', { name: 'Edit Topic' }))
    expect(screen.queryByRole('alert')).not.toBeInTheDocument()
  })

  it('does not leak Edit Topic errors into Create Topic', async () => {
    vi.spyOn(api, 'getSubject').mockResolvedValue(subject)
    vi.spyOn(api, 'listTopics').mockResolvedValue({ items: [topic], page: 0, size: 12, totalElements: 1, totalPages: 1 })
    vi.spyOn(api, 'updateTopic').mockRejectedValue(new ApiError({ kind: 'http', status: 400, code: 'VALIDATION_FAILED', message: 'hidden' }))
    renderAt(`/subjects/${subject.id}`, true)
    fireEvent.click(await screen.findByRole('button', { name: 'Edit Topic' }))
    fireEvent.change(screen.getByLabelText('Name'), { target: { value: 'Edited Topic' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save Topic' }))
    expect(await screen.findByRole('alert')).toHaveTextContent('Review the form')
    fireEvent.click(screen.getByRole('button', { name: 'Cancel' }))
