...
describe('materials API contracts', () => {
  ...
  it('parses processing without a synthetic update timestamp', async () => { transport.json.mockResolvedValue(processing); await expect(getMaterialProcessing(id)).resolves.toEqual(processing) })
  it('rejects the old processing timestamp contract', async () => { transport.json.mockResolvedValue({ ...processing, updatedAt: '2026-09-01T10:00:01Z' }); await expect(getMaterialProcessing(id)).rejects.toMatchObject({ code: 'INVALID_RESPONSE' }) })
